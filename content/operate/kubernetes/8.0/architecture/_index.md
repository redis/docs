...
Redis Enterprise Software for Kubernetes supports [Redis Flex]({{<relref "/operate/kubernetes/8.0/re-clusters/redis-flex">}}) (previously known as Redis on Flash), which extends your node memory to use both RAM and flash storage. SSDs (solid state drives) can store infrequently used (warm) keys and values while frequently used (hot) keys and values are stored in RAM. This improves performance and lowers costs for large datasets.
