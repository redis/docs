...
- operate
- kubernetes
description: The Redis Enterprise K8s 6.4.2-4 release supports Redis Enterprise Software
  6.4.2 and contains new features and feature improvements.
...
* Resharding fails for rack-aware databases with no replication (RED-97971)
  ...
  Workaround:
    Before resharding your database, [turn off rack awareness]({{< relref "/operate/kubernetes/reference/db-options.md" >}}). Set the `spec.rackAware` field in your REDB custom resource to `false`.
    After the resharding process is complete, you can [re-enable rack awareness]({{< relref "/operate/kubernetes/reference/db-options.md" >}}).
