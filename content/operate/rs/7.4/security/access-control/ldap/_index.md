...
Redis Enterprise Software supports [Lightweight Directory Access Protocol](https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol) (LDAP) authentication and authorization through its [role-based access controls](/content/operate/rs/7.4/security/access-control/_index.md) (RBAC).  You can use LDAP to authorize access to the Cluster Manager UI and to control database access.

You can configure LDAP roles using the Redis Enterprise Cluster Manager UI or [REST API](/content/operate/rs/7.4/references/rest-api/requests/ldap_mappings/_index.md).
...
1. A Redis Enterprise access control role for each LDAP group. Before you enable LDAP, you need to set up [role-based access controls](/content/operate/rs/7.4/security/access-control/_index.md) (RBAC).
...
3.  Update database access control lists (ACLs) to [authorize role access](/content/operate/rs/7.4/security/access-control/ldap/update-database-acls.md).  

If you already have appropriate roles, you can update them to include LDAP groups.

## More info

- Enable and configure [role-based LDAP](/content/operate/rs/7.4/security/access-control/ldap/enable-role-based-ldap.md)
- Map LDAP groups to [access control roles](/content/operate/rs/7.4/security/access-control/ldap/map-ldap-groups-to-roles.md)
- Update database ACLs to [authorize LDAP access](/content/operate/rs/7.4/security/access-control/ldap/update-database-acls.md)
- Learn more about Redis Enterprise Software [security and practices](/content/operate/rs/7.4/security/_index.md)
