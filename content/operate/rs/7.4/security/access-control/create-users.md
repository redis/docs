...
1. Review the [access control overview](/content/operate/rs/7.4/security/access-control/_index.md) to learn how to use role-based access control (RBAC) to manage users' cluster access and database access.

1. Create roles you can assign to users. See [Create roles with cluster access only](/content/operate/rs/7.4/security/access-control/create-cluster-roles.md), [Create roles with database access only](/content/operate/rs/7.4/security/access-control/create-db-roles.md), or [Create roles with combined access](/content/operate/rs/7.4/security/access-control/create-combined-roles.md) for instructions.
...
1. From the **Access Control > Users** tab in the Cluster Manager UI, you can:
    ...
    - Select **+ Add user** to [create a new user](/content/operate/rs/7.4/security/access-control/create-users.md).
...
- [Connect to a database](/content/operate/rs/7.4/databases/connect/_index.md) associated with the role and run limited Redis commands, depending on the role's Redis ACLs.

- Sign in to the Redis Enterprise Software Cluster Manager UI.

- Make a [REST API](/content/operate/rs/7.4/references/rest-api/_index.md) request.
