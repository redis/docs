...
- [Rack/Zone
Awareness](/content/operate/rs/7.4/clusters/configure/rack-zone-awareness.md) - When rack-zone awareness is used additional logic ensures that master and replica shards never share the same rack, thus ensuring availability even under loss of an entire rack.
- [High Availability for Replica Shards](/content/operate/rs/7.4/databases/configure/replica-ha.md) - When high availability
...
> as explained in [Database memory limits](/content/operate/rs/7.4/databases/memory-performance/memory-limit.md).
...
Redis databases that use [replication for high availability](/content/operate/rs/7.4/databases/durability-ha/replication.md) maintain a replication backlog (per shard) to synchronize the primary and replica shards of a database.
...
Use the [`rladmin`](/content/operate/rs/7.4/references/cli-utilities/rladmin/_index.md) and the [`crdb-cli`](/content/operate/rs/7.4/references/cli-utilities/crdb-cli/_index.md) utilities to control the size of the replication backlog. You can set it to `auto` or set a specific size.  
...
Use the [`crdb-cli`](/content/operate/rs/7.4/references/cli-utilities/crdb-cli/_index.md) utility to control the size of the CRDT replication backlog. You can set it to `auto` or set a specific size:  
...
To control the size of the replication log, use [`rladmin`](/content/operate/rs/7.4/references/cli-utilities/rladmin/_index.md) to tune the local database instance in each cluster.
```text
