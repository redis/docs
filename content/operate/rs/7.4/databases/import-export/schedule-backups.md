...
To make an on-demand backup, [export your data](/content/operate/rs/7.4/databases/import-export/export-data.md).
...
The backup process creates compressed (.gz) RDB files that you can [import into a database](/content/operate/rs/7.4/databases/import-export/import-data.md).
...
> - To limit the parallel backup for shards, set both [`tune cluster max_simultaneous_backups`](/content/operate/rs/7.4/references/cli-utilities/rladmin/tune.md#tune-cluster) and [`tune node max_redis_forks`](/content/operate/rs/7.4/references/cli-utilities/rladmin/tune.md#tune-node). `max_simultaneous_backups` is set to 4 by default.
...
When saved to a local mount point or a cloud provider, backup locations need to be available to [the group and user](/content/operate/rs/7.4/installing-upgrading/install/customize-user-and-group.md) running Redis Enterprise Software, `redislabs:redislabs` by default.  
