...
|-----------|------------|-------------|
| audit_address | string | TCP/IP address where a listener can capture [audit event notifications](/content/operate/rs/7.4/security/audit-events.md) |
| audit_port | string | Port where a listener can capture [audit event notifications](/content/operate/rs/7.4/security/audit-events.md) |
| audit_protocol | `tcp`<br/>`local` | Protocol used for [audit event notifications](/content/operate/rs/7.4/security/audit-events.md)<br/>For production systems, only `tcp` is supported. |
