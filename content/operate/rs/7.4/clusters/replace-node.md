...
1.  Install and
    configure Redis Enterprise Software on the node. See [Install and setup](/content/operate/rs/7.4/installing-upgrading/_index.md) for more information.

    > [!NOTE]
    > If you are using [Auto Tiering](/content/operate/rs/7.4/databases/auto-tiering/_index.md), make sure the required flash storage is set up on this new node.
    >

1. [Add the node](/content/operate/rs/7.4/clusters/add-node.md) to the cluster. Make sure the new node has as much available memory as the faulty
    node.
...
1. A message will appear informing you that the cluster has a faulty node
    ...
    > - Any existing [DNS records](/content/operate/rs/7.4/networking/cluster-dns.md) must be updated
