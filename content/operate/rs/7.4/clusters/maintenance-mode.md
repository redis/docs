...
1. Marks the node as a quorum node to prevent shards and endpoints from migrating to it.
    ...
    At this point, [`rladmin status`](/content/operate/rs/7.4/references/cli-utilities/rladmin/status.md) displays the node's shards field in yellow, which indicates that shards cannot migrate to the node.
...
When you turn off maintenance mode, you can restore the node configuration from a maintenance mode snapshot or any snapshots previously created by [`rladmin node <node_id> snapshot create`](/content/operate/rs/7.4/references/cli-utilities/rladmin/node/snapshot.md#node-snapshot-create). If you do not specify a snapshot, Redis Enterprise uses the latest maintenance mode snapshot by default.
...
This example shows how the output of [`rladmin status`](/content/operate/rs/7.4/references/cli-utilities/rladmin/status.md) changes when you turn on maintenance mode for a node.
...
You can also turn maintenance mode on or off using [REST API requests](/content/operate/rs/7.4/references/rest-api/_index.md) to [<nobr>POST `/nodes/{node_uid}/actions/{action}`</nobr>](/content/operate/rs/7.4/references/rest-api/requests/nodes/actions.md#post-node-action).
...
You can send a request to [<nobr>GET `/nodes/{node_uid}/actions/{action}`</nobr>](/content/operate/rs/7.4/references/rest-api/requests/nodes/actions.md#get-node-action) to track the [status](/content/operate/rs/7.4/references/rest-api/objects/action.md) of the `maintenance_on` and `maintenance_off` actions.
