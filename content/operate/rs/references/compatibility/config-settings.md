---
Title: Compatibility with source available Redis configuration settings
alwaysopen: false
categories:
- docs
- operate
- rs
description: Source available Redis configuration settings supported by Redis Enterprise.
linkTitle: Configuration settings
weight: 50
---

Redis Enterprise Software and [Redis Cloud]({{< relref "/operate/rc" >}}) only support a subset of [source available Redis configuration settings]({{<relref "/operate/oss_and_stack/management/config">}}). Using [`CONFIG GET`]({{< relref "/commands" >}}/config-get/) or [`CONFIG SET`]({{< relref "/commands" >}}/config-set/) with unsupported configuration settings returns an error.
