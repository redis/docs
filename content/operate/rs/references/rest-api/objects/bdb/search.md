...
| search-tiered-hnsw-buffer-limit | integer (range: 0-9223372036854775807) (default: 1024) | Use for setting the buffer limit threshold for vector similarity tiered HNSW index, so that if we are using WORKERS for indexing, and the number of vectors waiting in the buffer to be indexed exceeds this limit, we insert new vectors directly into HNSW. Requires a database restart to take effect. |
