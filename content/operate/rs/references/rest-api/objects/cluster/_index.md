...
| <span class="break-all">control_plane_basic_authentication</span> | boolean (default: true) | Enable Basic and Digest authentication on inbound cluster management API requests. When `false`, the cluster rejects Basic and Digest authentication; clients must use certificate or JWT authentication. See [Disable basic authentication for the REST API]({{<relref "/operate/rs/security/access-control/disable-basic-authentication">}}). |
