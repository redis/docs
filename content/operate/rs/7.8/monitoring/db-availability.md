...
If the database is unavailable, returns an error status code and a JSON object that contains [`error_code` and `description` fields](/content/operate/rs/7.8/references/rest-api/requests/bdbs/availability.md#get-db-error-codes).
...
If the local database endpoint is unavailable, returns an error status code and a JSON object that contains [`error_code` and `description` fields](/content/operate/rs/7.8/references/rest-api/requests/bdbs/availability.md#get-endpoint-error-codes).

## Availability by database status

The following table shows the relationship between a database's status and availability. For more details about the database status values, see [BDB status field](/content/operate/rs/7.8/references/rest-api/objects/bdb/status.md).
