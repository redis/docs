...
By default, Redis Enterprise Software is installed with the user:group `redislabs:redislabs`. See [Access control](/content/operate/rs/7.8/security/access-control/_index.md) for user and group security information.
...
To customize the user or group during [installation](/content/operate/rs/7.8/installing-upgrading/install/install-on-linux.md), include the `--os-user` or `--os-group` [command-line options](/content/operate/rs/7.8/installing-upgrading/install/install-script.md) when you run the `install.sh` script. For example:
