...
1. [Add the new node](/content/operate/rs/7.8/clusters/add-node.md) to the existing cluster.

1. [Remove a node](/content/operate/rs/7.8/clusters/remove-node.md#remove-a-node) with the old socket file location.
...
1. Use Replica Of to migrate your databases from the original cluster to the new cluster. For detailed steps, see the procedure to [configure Replica Of with different Redis Enterprise clusters](/content/operate/rs/7.8/databases/import-export/replica-of/create.md#different-cluster).
