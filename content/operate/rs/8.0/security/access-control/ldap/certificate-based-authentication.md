...
1. [Enable mutual TLS](/content/operate/rs/8.0/security/encryption/tls/enable-tls.md) for the relevant databases to set up certificate-based authentication.

1. [Set up LDAP for the cluster](/content/operate/rs/8.0/security/access-control/ldap/enable-role-based-ldap.md#set-up-ldap-connection).

1. [Map LDAP users to appropriate cluster roles](/content/operate/rs/8.0/security/access-control/ldap/map-ldap-groups-to-roles.md).

1. [Configure database ACLs](/content/operate/rs/8.0/security/access-control/ldap/update-database-acls.md) to authorize LDAP users.

## Set up certificate-based authentication for LDAP

To set up certificate-based authentication for LDAP:

1. Enable certificate-based authentication in the cluster LDAP configuration using an [update LDAP configuration](/content/operate/rs/8.0/references/rest-api/api-reference.md#tag/Cluster/operation/cluster_update_ldap_config) REST API request.
...
1. Enable external certificate-based authentication in cluster settings using an [update cluster settings](/content/operate/rs/8.0/references/rest-api/requests/cluster/_index.md#put-cluster) REST API request:
...
After configuration is complete, try to [connect to the database with certificate-based authentication](/content/operate/rs/8.0/security/certificates/certificate-based-authentication.md#authenticate-database-connections) and verify your setup.
