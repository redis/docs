...
Roles with database access grant the ability to access and interact with a database's data. Database access privileges are determined by defining [Redis ACLs](/content/operate/rs/8.0/security/access-control/redis-acl-overview.md) and adding them to roles.
...
1. [Update a database's configuration](/content/operate/rs/8.0/references/rest-api/requests/bdbs/_index.md#put-bdbs) to add `roles_permissions` with the role and Redis ACL:
...
You can [assign the new role to users](/content/operate/rs/8.0/security/access-control/create-users.md#assign-roles-to-users) to grant database access.
