...
To use IPv6 for internal traffic on a machine with both IPv4 and IPv6 interfaces, set `use_internal_ipv6` to `true` when you create a cluster using the [bootstrap REST API request](/content/operate/rs/8.0/references/rest-api/requests/bootstrap/_index.md#post-bootstrap):
...
1. Turn the node into a replica using [`rladmin`](/content/operate/rs/8.0/references/cli-utilities/rladmin/node/enslave.md):
...
1. From a different cluster node, use [`rladmin node addr set`](/content/operate/rs/8.0/references/cli-utilities/rladmin/node/addr.md) to update the first node's IP address:
...
You can use [rladmin node external_addr](/content/operate/rs/8.0/references/cli-utilities/rladmin/node/external-addr.md) to change a node's external IP addresses.
