...
Some Redis Software [REST API requests](/content/operate/rs/8.0/references/rest-api/requests/_index.md) may require the user to have specific permissions.

Administrators can assign a predefined role to a user with the [Cluster Manager UI](/content/operate/rs/8.0/security/access-control/create-users.md) or a [<nobr>`PUT /v1/users/{uid}`</nobr> API request](/content/operate/rs/8.0/references/rest-api/requests/users/_index.md#put-user) to grant necessary permissions to them.
