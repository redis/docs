...
- Send a [`GET /nodes` REST API request](/content/operate/rs/8.0/references/rest-api/requests/nodes/_index.md) and see `supported_database_versions` in the response.
...
- For [Replica Of databases](/content/operate/rs/8.0/databases/import-export/replica-of/_index.md), upgrade the destination database before the source database.

- Check client compatibility with the database version.

    If you run Redis Stack commands with Go-Redis versions 9 and later or Lettuce versions 6 and later, set the client’s protocol version to RESP2 before upgrading your database to Redis version 7.2 to prevent potential application issues due to RESP3 breaking changes. See [Client prerequisites for Redis 7.2 upgrade](/content/operate/rs/8.0/references/compatibility/resp.md#client-prerequisites-for-redis-72-upgrade) for more details and examples.

- To avoid data loss during the upgrade, [back up your data](/content/operate/rs/8.0/databases/import-export/schedule-backups.md).  

    You can [export the data](/content/operate/rs/8.0/databases/import-export/export-data.md) to an external location, [enable replication](/content/operate/rs/8.0/databases/durability-ha/replication.md), or [enable persistence](/content/operate/rs/8.0/databases/configure/database-persistence.md).
...
1. Use an [upgrade database](/content/operate/rs/8.0/references/rest-api/requests/bdbs/upgrade.md) REST API request. During the upgrade process, the database will restart without losing any data. Use the `preserve_roles` option to keep the database's current state, including primary shard placement, and prevent the cluster from becoming unbalanced.
    ...
    For additional database upgrade options, see the [request body](/content/operate/rs/8.0/references/rest-api/requests/bdbs/upgrade.md#request-body) section of the database upgrade requests reference.
