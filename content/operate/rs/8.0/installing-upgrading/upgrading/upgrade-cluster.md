...
See the [Redis Software product lifecycle](/content/operate/rs/8.0/installing-upgrading/product-lifecycle.md) for more information about release numbers and the end-of-life schedule.
...
- Verify access to [rlcheck](/content/operate/rs/8.0/references/cli-utilities/rlcheck/_index.md) and [rladmin](/content/operate/rs/8.0/references/cli-utilities/rladmin/_index.md#use-the-rladmin-shell) commands.

- Run [rlcheck](/content/operate/rs/8.0/references/cli-utilities/rlcheck/_index.md) on each node and verify there are no issues:

    ```sh
    rlcheck
    ```

- Verify [maintenance mode](/content/operate/rs/8.0/clusters/maintenance-mode.md) is not enabled:

    1. On a node in the cluster, run [`rladmin status`](/content/operate/rs/8.0/references/cli-utilities/rladmin/status.md):
...
- Before you upgrade a cluster from Redis Software version 6.2.x to 7.8.x, you must follow these steps if the cluster has any databases with Redis version 6.0:
    ...
    1. [Upgrade Redis 6.0 databases](/content/operate/rs/8.0/installing-upgrading/upgrading/upgrade-database.md) to Redis 6.2.

- [Upgrade your databases](/content/operate/rs/8.0/installing-upgrading/upgrading/upgrade-database.md) to a version that is supported by the target Redis Software version before upgrading the cluster. We recommend you upgrade the databases to the latest supported version if possible. Make sure to test the upgrade in a non-production environment to determine any impact.
...
- Upgrade the cluster's primary (master) node first. To identify the primary node, use one of the following methods:
    ...
    - [`rladmin status nodes`](/content/operate/rs/8.0/references/cli-utilities/rladmin/status.md#status-nodes) command
    
    - [`GET /nodes/status`](/content/operate/rs/8.0/references/rest-api/requests/nodes/status.md#get-all-nodes-status) REST API request
...
1.  Run the install command. See [installation script options](/content/operate/rs/8.0/installing-upgrading/install/install-script.md) for a list of command-line options you can add to the following command. You cannot use options marked as "new installs only" during an in-place upgrade.
...
1. [Install a later version of Redis Software](/content/operate/rs/8.0/installing-upgrading/install/install-on-linux.md) on a new node.

1. [Add the new node](/content/operate/rs/8.0/clusters/add-node.md) to the cluster.

1. If the [cluster uses DNS](/content/operate/rs/8.0/networking/cluster-dns.md), add the new node’s IP address to the DNS records.

1. [Promote the first new node](/content/operate/rs/8.0/clusters/change-node-role.md#promote-secondary-node) to become the primary node.

1. [Remove one node](/content/operate/rs/8.0/clusters/remove-node.md#remove-a-node) running the earlier Redis Software version from the cluster.

1. Repeat the previous steps until all nodes with the earlier Redis Software version are removed. If the final node to remove from the cluster is the primary node, [demote it](/content/operate/rs/8.0/clusters/change-node-role.md#demote-primary-node) to a secondary node before you remove it.

### Replace node upgrade method {#replace-node-upgrade}

1. Complete all [prerequisites](#upgrade-prerequisites) before starting the rolling upgrade.

1. [Remove a node](/content/operate/rs/8.0/clusters/remove-node.md#remove-a-node) with the earlier Redis Software version from the cluster.
...
1. [Install a later version of Redis Software](/content/operate/rs/8.0/installing-upgrading/install/install-on-linux.md) on the removed node or a new node.

1. [Add the new node](/content/operate/rs/8.0/clusters/add-node.md) to the cluster.

    If you want to reuse the removed node's ID when you add the node to the cluster, run [`rladmin cluster join`](/content/operate/rs/8.0/references/cli-utilities/rladmin/cluster/join.md) with the `replace_node` flag:
...
1. Verify node health:
    ...
    1. Run [`rladmin status`](/content/operate/rs/8.0/references/cli-utilities/rladmin/status.md) on the new node:
...
1. Repeat the previous steps until all nodes with the earlier Redis Software version are replaced. If the final node to remove from the cluster is the primary node, [demote it](/content/operate/rs/8.0/clusters/change-node-role.md#demote-primary-node) to a secondary node before you remove it.
