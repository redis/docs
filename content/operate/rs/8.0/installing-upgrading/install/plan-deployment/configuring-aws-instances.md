...
persistence storage](/content/operate/rs/8.0/installing-upgrading/install/plan-deployment/persistent-ephemeral-storage.md) is configured to use this volume.

> [!NOTE]
> After [installing the Redis Software package](/content/operate/rs/8.0/installing-upgrading/_index.md) on the instance
> and **before** running through [the setup process](/content/operate/rs/8.0/clusters/new-cluster-setup.md),
...
1. When using [Redis Flex or Auto Tiering](/content/operate/rs/8.0/databases/flash/_index.md)
...
requirements](/content/operate/rs/8.0/installing-upgrading/install/plan-deployment/hardware-requirements.md).
...
- If you are using the DNS resolving option with Redis Software, define a DNS UDP
    ...
    using the [DNS resolving mechanism](/content/operate/rs/8.0/networking/cluster-dns.md).
- To create a cluster that has multiple nodes all running as instances on AWS,
    ...
    To limit the number of open ports, you can open only the [ports used by Redis Software](/content/operate/rs/8.0/networking/port-configurations.md).

After successfully launching the instances:

1. Install Redis Software from the [Linux package or AWS AMI](/content/operate/rs/8.0/installing-upgrading/_index.md).
2. [Set up the cluster](/content/operate/rs/8.0/clusters/new-cluster-setup.md).
