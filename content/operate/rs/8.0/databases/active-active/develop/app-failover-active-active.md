...
> [!NOTE]
> For other disaster recovery strategies including network-based, proxy-based, and client library approaches, see [Active-Active disaster recovery strategies](/content/operate/rs/8.0/databases/active-active/disaster-recovery/_index.md).
...
You can also use [database availability API requests](/content/operate/rs/8.0/monitoring/db-availability.md) to determine if a database replica is available to handle read and write operations. The lag-aware database availability requests considers CRDT replication lag as a health check criterion to prevent reading stale data during failback scenarios.
