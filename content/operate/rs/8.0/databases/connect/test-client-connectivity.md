...
1. After you [create a Redis database](/content/operate/rs/8.0/databases/create.md), copy the database endpoint, which contains the cluster name (FQDN).
...
1. If the database does not respond, try to connect to the database
    ...
    [Configure cluster DNS](/content/operate/rs/8.0/networking/cluster-dns.md).
...
- [`redis-cli`](/content/operate/rs/8.0/references/cli-utilities/redis-cli/_index.md), the built-in command-line tool
...
For more `redis-cli` connection examples, see the [`redis-cli` reference](/content/operate/rs/8.0/references/cli-utilities/redis-cli/_index.md).
...
1. Select **Use TLS** if [TLS](/content/operate/rs/8.0/security/encryption/tls/_index.md) is set up.
