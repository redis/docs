...
Review [OSS Cluster API](/content/operate/rs/8.0/clusters/optimize/oss-cluster-api.md) to determine if you should enable this feature for your database.
...
- Use the standard [hashing policy](/content/operate/rs/8.0/databases/durability-ha/clustering.md#supported-hashing-policies).
- Have the [proxy policy](/content/operate/rs/8.0/databases/configure/proxy-policy.md) set to either _All primary shards_ or _All nodes_.

In addition, the database must _not_:

- Use node `include` or `exclude` in the proxy policy.
- Use [RedisTimeSeries](/content/operate/oss_and_stack/stack-with-enterprise/timeseries/_index.md) or [RedisGears](/content/operate/oss_and_stack/stack-with-enterprise/deprecated-features/gears-v1/_index.md) modules.

The OSS Cluster API setting applies to individual databases instead of the entire cluster.

> [!NOTE]
> For Kubernetes deployments, see [Enable cluster-aware clients (OSS Cluster API)](/content/operate/kubernetes/networking/cluster-aware-clients.md).
...
When you use the Cluster Manager UI to enable the OSS Cluster API, it automatically configures the [prerequisites](/content/operate/rs/8.0/databases/configure/oss-cluster-api.md#prerequisites).
...
You can use the [`rladmin` utility](/content/operate/rs/8.0/references/cli-utilities/rladmin/_index.md) to enable the OSS Cluster API for Redis Software databases, including Replica Of databases.

For Active-Active (CRDB) databases, [use the crdb-cli utility](#active-active-databases).

Ensure the [prerequisites](/content/operate/rs/8.0/databases/configure/oss-cluster-api.md#prerequisites) have been configured.  Then, enable the OSS Cluster API for a Redis database from the command line:
...
You can enable the OSS Cluster API when you [create a database](/content/operate/rs/8.0/references/rest-api/requests/bdbs/_index.md#post-bdbs-v1) using the REST API:
...
To enable the OSS Cluster API for an existing database, you can use an [update database configuration](/content/operate/rs/8.0/references/rest-api/requests/bdbs/_index.md#put-bdbs) REST API request:
...
See the [`crdb-cli crdb create`](/content/operate/rs/8.0/references/cli-utilities/crdb-cli/crdb/create.md) reference for more options.
...
By default, using [`CLUSTER SLOTS`](/content/commands/cluster-slots.md) and [`CLUSTER SHARDS`](/content/commands/cluster-shards.md) in a Redis Software cluster exposes the internal IP addresses for databases with the OSS Cluster API enabled.
...
To verify that your database meets this requirement, make sure that the `CLUSTER KEYSLOT` reply is the same for all keys affected by the multi-key command.  To learn more, see [multi-key operations](/content/operate/rs/8.0/databases/durability-ha/clustering.md#multikey-operations).
