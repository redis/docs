...
You can configure [email alerts from the cluster](/content/operate/rs/8.0/monitoring/v1_monitoring.md#cluster-alerts) to notify you of cluster changes, including when a node is removed.
...
node](/content/operate/rs/8.0/clusters/replace-node.md).
...
cluster](/content/operate/rs/8.0/clusters/add-node.md).
...
(CLI)](/content/operate/rs/8.0/references/cli-utilities/rladmin/_index.md).

> [!NOTE]
> The [DNS records](/content/operate/rs/8.0/networking/cluster-dns.md) must be updated each time a node is added or replaced.
...
To remove a node using the REST API, use [`POST /v1/nodes/<node_id>/actions/remove`](/content/operate/rs/8.0/references/rest-api/requests/nodes/actions.md#post-node-action).
...
> you must [uninstall](/content/operate/rs/8.0/installing-upgrading/uninstalling.md)
> and [reinstall](/content/operate/rs/8.0/installing-upgrading/_index.md) the software on that node.
