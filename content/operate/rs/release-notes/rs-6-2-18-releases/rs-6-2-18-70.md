...
- Added a new script to generate new self-signed certificates for renewal. For details, see [Certificates](/content/operate/rs/security/certificates/_index.md)
- Added `rpm -q` command output to the logs (RS82673)
- Added logs for server operations: `start`, `stop`, `restart` (RS87775)
- Added `alert_mgr` to the [services you can deactivate with `rladmin`](/content/operate/rs/clusters/optimize/turn-off-services.md) (RS89572)
