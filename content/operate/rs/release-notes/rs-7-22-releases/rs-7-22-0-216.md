...
- Added the `license_expiration_days` metric to the [metrics stream engine](/content/operate/rs/monitoring/metrics_stream_engine/_index.md). This metric tracks the number of days until the [cluster license expires](/content/operate/rs/clusters/configure/license-keys.md#expired-cluster-license).
