...
Some CVEs announced for Redis do not affect Redis Enterprise due to different or additional functionality in Redis Enterprise compared to source available Redis.
...
- (CVE-2022-36021) String matching commands (like `SCAN` or `KEYS`) with a specially crafted pattern can trigger a denial-of-service attack on Redis, causing it to hang and consume 100% CPU time. (Redis 6.0.18)
