...
- Added a `replication` query parameter to [`GET /v1/bdbs/<uid>/actions/optimize_shards_placement`]({{<relref "/operate/rs/references/rest-api/requests/bdbs/actions/optimize_shards_placement">}}) REST API requests to return the database's optimized shards placement if you turn replication on or off.

- Added `shard_uid` and `shard_role` for each persistence file in the response body of [`GET /v1/bdbs/<uid>/actions/recover`]({{<relref "/operate/rs/references/rest-api/requests/bdbs/actions/recover">}}) REST API requests. Using the persistence files from the primary shards for recovery is recommended.
