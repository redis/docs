...
Improved distribution of [email alerts]({{< relref "/operate/rs/administering/designing-production/access-control" >}}) for cluster or database alerts.
...
You can now fine tune the [TLS configuration]({{< relref "/operate/rs/security/tls-ssl" >}}), and ease certificates management by excluding client authentication enforcement,
...
- Node upgrade fails if the SSL certificates were configured in version 5.0.2 and above by updating the certificates on the disk instead of using the new API.
    For assistance with this issue, see [SSL/TLS certificate updates]({{< relref "/operate/rs/administering/cluster-operations/updating-certificates" >}}) in our documentation or [contact Redis support](https://redislabs.com/company/support/).
