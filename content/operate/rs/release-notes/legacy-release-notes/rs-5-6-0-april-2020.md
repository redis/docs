...
RS 5.6.0 adds support for [Auto Tiering]({{< relref "/operate/rs/databases/auto-tiering/" >}}) databases with [Redis Modules](https://docs.redis.com/latest/modules). [RedisJSON](https://docs.redis.com/latest/modules/redisjson) is the first module to be available to run on a database with Auto Tiering enabled.
