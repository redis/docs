---
Title: Redis Software release notes 8.0.20-16 (May 2026)
alwaysopen: false
categories:
- docs
- operate
...
description: Certificate-based authentication for LDAP. Amazon Linux 2023 support. API enhancements for Active-Active databases and identifying manually stopped database traffic. New Redis Search metrics.
linkTitle: 8.0.20-16 (May 2026)
weight: 81
---
...
Redis Software now supports certificate-based authentication for LDAP users. This feature allows users to authenticate to the database using client certificates while authorization and role mapping are managed using LDAP.
...
- Setting the `mask_bdb_credentials` [cluster setting]({{<relref "/operate/rs/references/rest-api/requests/cluster#put-cluster">}}) to `true` also sanitizes responses for [Active-Active database]({{<relref "/operate/rs/references/rest-api/requests/crdbs">}}) REST API requests.
...
- The [metrics stream engine]({{<relref "/operate/rs/monitoring/metrics_stream_engine">}}) exports new Redis Search [metrics]({{<relref "/operate/rs/monitoring/metrics_stream_engine/prometheus-metrics-v2#shard-metrics">}}).
...
| Package | SHA256 checksum (8.0.20-16 May release) |
|---------|---------------------------------------|
...
| Amazon Linux 2023 | <span class="break-all"></span> |
...
Redis Software 8.0.20-16 supports Redis Open Source 8.6, 8.4, 8.2, 8.0, 7.4, 7.2, and 6.2. Below is the list of Redis Open Source CVEs and other security vulnerabilities fixed by version.
