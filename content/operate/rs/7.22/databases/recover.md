...
1. [Restore the cluster configuration](/content/operate/rs/7.22/clusters/cluster-recovery.md) from the CCS files
...
These files are stored in the [persistence storage location](/content/operate/rs/7.22/installing-upgrading/install/plan-deployment/persistent-ephemeral-storage.md).

The database recovery process includes:

1. If the cluster failed, [recover the cluster](/content/operate/rs/7.22/clusters/cluster-recovery.md).
...
- Before you start database recovery, make sure that the cluster that hosts the database is healthy.
    ...
    you must [recover the cluster](/content/operate/rs/7.22/clusters/cluster-recovery.md) before you recover the databases.
...
you can run the recovery process from the [`rladmin`](/content/operate/rs/7.22/references/cli-utilities/rladmin/_index.md)
...
1. To see which databases are recoverable, run [`rladmin recover list`](/content/operate/rs/7.22/references/cli-utilities/rladmin/recover.md#recover-list):
...
Run [`rladmin status modules`](/content/operate/rs/7.22/references/cli-utilities/rladmin/status.md#status-modules):
...
Send a [`GET /v1/modules`](/content/operate/rs/7.22/references/rest-api/requests/modules/_index.md#list-modules) request:
...
```
    ...
    1. Identify the module versions required by the recoverable databases. To do so, run [`rladmin status modules`](/content/operate/rs/7.22/references/cli-utilities/rladmin/status.md#status-modules) and check the `DATABASE MODULES` section for the module versions listed for the recoverable databases:
...
1. Add the module to the cluster with a [`POST` request to the `/v2/modules`](/content/operate/rs/7.22/references/rest-api/requests/modules/_index.md#post-module-v2) endpoint:
...
1. If the module installation succeeds, the `POST` request returns a [JSON object](/content/operate/rs/7.22/references/rest-api/objects/module.md) that represents the new module. If it fails, it may return a JSON object with an `error_code` and `description` with more details.

        {{< /multitabs >}}

1. Recover the database using one of the following [`rladmin recover`](/content/operate/rs/7.22/references/cli-utilities/rladmin/recover.md) commands:
...
To enable automatic recovery using `rladmin`, run the [`rladmin tune cluster`](/content/operate/rs/7.22/references/cli-utilities/rladmin/tune.md#tune-cluster) command:
...
To enable automatic recovery using the REST API, use an [update cluster policy](/content/operate/rs/7.22/references/rest-api/requests/cluster/policy.md#put-cluster-policy) request:
...
You can also set `recovery_wait_time` when you [create a database](/content/operate/rs/7.22/references/rest-api/requests/bdbs/_index.md#post-bdbs-v1) using the REST API.
