...
Redis Enterprise Software uses [proxies](/content/operate/rs/7.22/references/terminology.md#proxy) to manage and optimize access to database shards. Each node in the cluster runs a single proxy process, which can be active (receives incoming traffic) or passive (waits for failovers).

You can configure default [proxy policies](/content/operate/rs/7.22/databases/configure/proxy-policy.md) to determine which nodes' proxies are active and bound to new databases by default.
...
The default [shard placement policy](/content/operate/rs/7.22/databases/memory-performance/shard-placement-policy.md) determines the distribution of database shards across nodes in the cluster.
...
New databases use the default Redis database version unless you select a different **Database version** when you [create a database](/content/operate/rs/7.22/databases/create.md) in the Cluster Manager UI or specify the `redis_version` in a [create database REST API request](/content/operate/rs/7.22/references/rest-api/requests/bdbs/_index.md).
...
Enable [internode encryption](/content/operate/rs/7.22/security/encryption/internode-encryption.md) to encrypt data in transit between nodes for new databases by default.
