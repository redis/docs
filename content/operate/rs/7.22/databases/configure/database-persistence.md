...
Data is stored in RAM or a combination of RAM and flash memory ([Auto Tiering](/content/operate/rs/7.22/databases/auto-tiering/_index.md)), which risks data loss during process or server failures. Redis Enterprise Software supports multiple methods to persist data to disk on a per-database basis to ensure data durability.
...
You can configure persistence when you [create a database](/content/operate/rs/7.22/databases/create.md), or you can edit an existing database's configuration:
