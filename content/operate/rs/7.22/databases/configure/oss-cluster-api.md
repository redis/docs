...
Review [OSS Cluster API](/content/operate/rs/7.22/clusters/optimize/oss-cluster-api.md) to determine if you should enable this feature for your database.
...
- Use the standard [hashing policy](/content/operate/rs/7.22/databases/durability-ha/clustering.md#supported-hashing-policies).
- Have the [proxy policy](/content/operate/rs/7.22/databases/configure/proxy-policy.md) set to either _All primary shards_ or _All nodes_.
...
When you use the Cluster Manager UI to enable the OSS Cluster API, it automatically configures the [prerequisites](/content/operate/rs/7.22/databases/configure/oss-cluster-api.md#prerequisites).
...
You can use the [`rladmin` utility](/content/operate/rs/7.22/references/cli-utilities/rladmin/_index.md) to enable the OSS Cluster API for Redis Enterprise Software databases, including Replica Of databases.

For Active-Active (CRDB) databases, [use the crdb-cli utility](#active-active-databases).

Ensure the [prerequisites](/content/operate/rs/7.22/databases/configure/oss-cluster-api.md#prerequisites) have been configured.  Then, enable the OSS Cluster API for a Redis database from the command line:
...
You can enable the OSS Cluster API when you [create a database](/content/operate/rs/7.22/references/rest-api/requests/bdbs/_index.md#post-bdbs-v1) using the REST API:
...
To enable the OSS Cluster API for an existing database, you can use an [update database configuration](/content/operate/rs/7.22/references/rest-api/requests/bdbs/_index.md#put-bdbs) REST API request:
...
See the [`crdb-cli crdb create`](/content/operate/rs/7.22/references/cli-utilities/crdb-cli/crdb/create.md) reference for more options.
...
To verify that your database meets this requirement, make sure that the `CLUSTER KEYSLOT` reply is the same for all keys affected by the multi-key command.  To learn more, see [multi-key operations](/content/operate/rs/7.22/databases/durability-ha/clustering.md#multikey-operations).
