...
See [Active-Active Redis](/content/operate/rs/7.22/databases/active-active/_index.md) for more information about geo-distributed replication. For more info on other high availability features, see [Durability and high availability](/content/operate/rs/7.22/databases/durability-ha/_index.md).

## Participating clusters

You need at least [two participating clusters](/content/operate/rs/7.22/clusters/new-cluster-setup.md) for an Active-Active database. If your database requires more than ten participating clusters, contact Redis support. You can [add or remove participating clusters](/content/operate/rs/7.22/databases/active-active/manage.md#participating-clusters/) after database creation.
...
Database memory limits define the maximum size of your database across all database replicas and [shards](/content/operate/rs/7.22/references/terminology.md#redis-instance-shard) on the cluster. Your memory limit also determines the number of shards.
...
- [**database clustering**](/content/operate/rs/7.22/databases/durability-ha/clustering.md): enables you to spread your data into shards across multiple nodes (scale out).
- [**database replication**](/content/operate/rs/7.22/databases/durability-ha/replication.md): enabling replication doubles memory consumption
- [**Active-Active replication**](/content/operate/rs/7.22/databases/active-active/_index.md): enabling Active-Active replication requires double the memory of regular replication, which can be up to two times (2x) the original data size per instance.
- [**database replication backlog**](/content/operate/rs/7.22/databases/active-active/manage.md#replication-backlog/) for synchronization between shards. By default, this is set to 1% of the database size.
- [**Active-Active replication backlog**](/content/operate/rs/7.22/databases/active-active/manage.md) for synchronization between clusters. By default, this is set to 1% of the database size.
...
For more information on memory limits, see [Memory and performance](/content/operate/rs/7.22/databases/memory-performance/_index.md) or [Database memory limits](/content/operate/rs/7.22/databases/memory-performance/memory-limit.md).
...
See [Synchronizing cluster node clocks](/content/operate/rs/7.22/clusters/configure/sync-clocks.md) for more information.
...
- An existing database can't be changed into an Active-Active database. To move data from an existing database to an Active-Active database, you must [create a new Active-Active database](/content/operate/rs/7.22/databases/active-active/create.md) and [migrate the data](/content/operate/rs/7.22/databases/import-export/migrate-to-active-active.md).
- [Discovery service](/content/operate/rs/7.22/databases/durability-ha/discovery-service.md) is not supported with Active-Active databases. Active-Active databases require FQDNs or [mDNS](/content/operate/rs/7.22/networking/mdns.md).
