...
Partial synchronization requires a backlog large enough to store the data operations until connection is restored. See [replication backlog](/content/operate/rs/7.22/databases/active-active/manage.md#replication-backlog) for more info on changing the replication backlog size.
...
- The [Active-Active replication backlog](/content/operate/rs/7.22/databases/active-active/manage.md#replication-backlog) is also sent to the replica during a full sync.
...
To restart a regular database's syncer after an unrecoverable error, [update the database configuration](/content/operate/rs/7.22/references/rest-api/requests/bdbs/_index.md#put-bdbs) with the REST API to enable `sync`:
...
-  For each participating cluster, [update the database configuration](/content/operate/rs/7.22/references/rest-api/requests/bdbs/_index.md#put-bdbs) with the REST API to enable `sync`:
...
- Run [`crdb-cli crdb update`](/content/operate/rs/7.22/references/cli-utilities/crdb-cli/crdb/update.md):
