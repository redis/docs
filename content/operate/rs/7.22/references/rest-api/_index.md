...
- Supported [request endpoints](/content/operate/rs/7.22/references/rest-api/requests/_index.md), organized by path
- Supported [objects](/content/operate/rs/7.22/references/rest-api/objects/_index.md), both request and response
- Built-in roles and associated [permissions](/content/operate/rs/7.22/references/rest-api/permissions.md)
- [Redis Enterprise Software REST API quick start](/content/operate/rs/7.22/references/rest-api/quick-start.md) with examples
...
For more examples, see the [Redis Enterprise Software REST API quick start](/content/operate/rs/7.22/references/rest-api/quick-start.md).

### Permissions

By default, the admin user is authorized for access to all endpoints. Use [role-based access controls](/content/operate/rs/7.22/security/access-control/_index.md) and [role permissions](/content/operate/rs/7.22/references/rest-api/permissions.md) to manage access.

If a user attempts to access an endpoint that is not allowed in their role, the request will fail with a [`403 Forbidden`](https://www.rfc-editor.org/rfc/rfc9110.html#name-403-forbidden) status code. For more details on which user roles can access certain endpoints, see [Permissions](/content/operate/rs/7.22/references/rest-api/permissions.md).

### Certificates

The Redis Enterprise Software REST API uses [Self-signed certificates](/content/operate/rs/7.22/security/certificates/_index.md) to ensure the product is secure. When you use the default self-signed certificates, the HTTPS requests will fail with `SSL certificate problem: self signed certificate` unless you turn off SSL certificate verification.
...
If you are using a [Redis Enterprise Software Docker image](/content/operate/rs/7.22/installing-upgrading/quickstarts/docker-quickstart.md), run the following command to start the Docker image with port 9443 exposed:
...
When an endpoint supports multiple versions, each version is documented on the corresponding endpoint.  For example, the [bdbs request](/content/operate/rs/7.22/references/rest-api/requests/bdbs/_index.md) page documents POST requests for [version 1](/content/operate/rs/7.22/references/rest-api/requests/bdbs/_index.md#post-bdbs-v1) and [version 2](/content/operate/rs/7.22/references/rest-api/requests/bdbs/_index.md#post-bdbs-v2).
