...
Use this action to resume read and write traffic on a database, where traffic was previously paused using the [`stop_traffic`](/content/operate/rs/7.22/references/rest-api/requests/bdbs/actions/stop_traffic.md) action.
...
Returns a JSON object with an `action_uid`. You can track the action's progress with a [`GET /v1/actions/<action_uid>`](/content/operate/rs/7.22/references/rest-api/requests/actions/_index.md#get-action) request.
