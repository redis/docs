...
The `replica_sources` status field relates to the [Replica Of](/content/operate/rs/7.22/databases/import-export/replica-of/create.md) feature, which enables the creation of a Redis database (single- or multi-shard) that synchronizes data from another Redis database (single- or multi-shard).
