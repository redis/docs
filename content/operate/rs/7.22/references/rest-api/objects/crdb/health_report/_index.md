...
| configurations | array of [health_report_configuration](/content/operate/rs/7.22/references/rest-api/objects/crdb/health_report/health_report_configuration.md) objects | Stored database configurations |
...
[{
  ...
  "status": string
}, ...] {{</code>}} | Connections to other clusters and their statuses. A replication link's `bdb_uid` is the unique ID of a local database instance ([bdb](/content/operate/rs/7.22/references/rest-api/objects/bdb/_index.md)) in the current cluster. The `replica_uid` is the unique ID of the database's remote replica, located in the connected cluster. |
