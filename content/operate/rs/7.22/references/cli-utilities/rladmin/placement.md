...
> [!NOTE]
> `rladmin placement db` is deprecated and will be removed in a future version. Use [`rladmin tune db`](/content/operate/rs/7.22/references/cli-utilities/rladmin/tune.md#tune-db) with the `shards_placement` parameter instead.
...
Use [`rladmin status databases`](/content/operate/rs/7.22/references/cli-utilities/rladmin/status.md#status-databases) to verify that the failover completed.
