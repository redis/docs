...
Sets the node's local recovery path, which specifies the directory where [persistence files](/content/operate/rs/7.22/databases/configure/database-persistence.md) are stored. You can use these persistence files to [recover a failed database](/content/operate/rs/7.22/databases/recover.md).
