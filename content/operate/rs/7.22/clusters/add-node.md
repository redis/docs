...
- The clocks on all nodes must always be [synchronized](/content/operate/rs/7.22/clusters/configure/sync-clocks.md).
...
- You must [update the DNS records](/content/operate/rs/7.22/networking/cluster-dns.md)
...
1. [Install the Redis Enterprise Software installation package](/content/operate/rs/7.22/installing-upgrading/_index.md) on a clean installation
    of a [supported operating system](/content/operate/rs/7.22/installing-upgrading/install/plan-deployment/supported-platforms.md).
...
1. To connect to the Cluster Manager UI of the new Redis Enterprise Software installation, go to: <https://URL or IP address:8443>
    ...
    > You can also [replace the TLS certificate](/content/operate/rs/7.22/security/certificates/updating-certificates.md)
...
- Command-line method:
    ...
    Run the [`rlcheck`](/content/operate/rs/7.22/references/cli-utilities/rlcheck/_index.md) utility from the node's command line.
