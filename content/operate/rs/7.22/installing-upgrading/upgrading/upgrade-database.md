...
- Send a [`GET /nodes` REST API request](/content/operate/rs/7.22/references/rest-api/requests/nodes/_index.md) and see `supported_database_versions` in the response.
...
- Review the relevant [release notes](/content/operate/rs/release-notes/_index.md) for any preparation instructions.
...
- Verify that both the [current database version and the target database version are supported](#db-versions-table) by the cluster's Redis Enterprise Software version.
    ...
    - _(Optional)_ Use the [`rladmin status extra all`](/content/operate/rs/7.22/references/cli-utilities/rladmin/status.md) command to display configuration details:
...
- For [Replica Of databases](/content/operate/rs/7.22/databases/import-export/replica-of/_index.md), upgrade the destination database before the source database.

- Check client compatibility with the database version.

    If you run Redis Stack commands with Go-Redis versions 9 and later or Lettuce versions 6 and later, set the client’s protocol version to RESP2 before upgrading your database to Redis version 7.2 to prevent potential application issues due to RESP3 breaking changes. See [Client prerequisites for Redis 7.2 upgrade](/content/operate/rs/7.22/references/compatibility/resp.md#client-prerequisites-for-redis-72-upgrade) for more details and examples.

- To avoid data loss during the upgrade, [back up your data](/content/operate/rs/7.22/databases/import-export/schedule-backups.md).  

    You can [export the data](/content/operate/rs/7.22/databases/import-export/export-data.md) to an external location, [enable replication](/content/operate/rs/7.22/databases/durability-ha/replication.md), or [enable persistence](/content/operate/rs/7.22/databases/configure/database-persistence.md).
...
1. Check the Redis database compatibility version for the database to confirm the upgrade.  
    ...
    - Use [`rladmin status databases extra all`](/content/operate/rs/7.22/references/cli-utilities/rladmin/status.md#status-databases) to display a list of the databases in your cluster and their current Redis database compatibility version:
