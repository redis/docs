...
| node_<node_uid>.rladmin | Information about the cluster's nodes, databases, endpoints, and shards. See [`rladmin status`](/content/operate/rs/7.22/references/cli-utilities/rladmin/status.md) for example output. |
...
If package creation fails with `internal error` or if you cannot access the UI, create a support package for the cluster from the command line on any node in the cluster using the [`rladmin cluster debug_info`](/content/operate/rs/7.22/references/cli-utilities/rladmin/cluster/debug_info.md) command: 
...
You can also use `debuginfo` [REST API](/content/operate/rs/7.22/references/rest-api/_index.md) requests to create and download support packages.
