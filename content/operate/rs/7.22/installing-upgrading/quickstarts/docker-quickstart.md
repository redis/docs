...
- Port 9443 for [REST API](/content/operate/rs/7.22/references/rest-api/_index.md) connections

- Port 12000 configured Redis database port allowing client connections

You can publish other [ports](/content/operate/rs/7.22/networking/port-configurations.md)
...
Every installation of Redis Enterprise Software includes the command-line tool [`redis-cli`](/content/operate/rs/7.22/references/cli-utilities/redis-cli/_index.md) to interact with your Redis database. You can use `redis-cli` to connect to your database from within the same Docker network.
...
The database you created uses port `12000`, which is also mapped from the Docker container back to the host environment. This lets you use any method you have available locally to [connect to a Redis database](/content/operate/rs/7.22/databases/connect/_index.md). Use `localhost` as the `host` and `12000` as the port.
