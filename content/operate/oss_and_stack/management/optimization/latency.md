...
1. Make sure you are not running slow commands that are blocking the server. Use the Redis [Slow Log feature](/content/commands/slowlog.md) to check this.
...
5. Enable and use the [Latency monitor](/content/operate/oss_and_stack/management/optimization/latency-monitor.md) feature of Redis in order to get a human readable description of the latency events and causes in your Redis instance.
...
ASAP. Please refer to the [Latency monitor documentation](/content/operate/oss_and_stack/management/optimization/latency-monitor.md).
...
like [`BGSAVE`](/content/commands/bgsave.md) or [`BGREWRITEAOF`](/content/commands/bgrewriteaof.md). These tasks must **never** run on the same core
...
normal commands, like [`GET`](/content/commands/get.md) or [`SET`](/content/commands/set.md) or [`LPUSH`](/content/commands/lpush.md) this is not a problem
at all since these commands are executed in constant (and very small) time.
However there are commands operating on many elements, like [`SORT`](/content/commands/sort.md), [`LREM`](/content/commands/lrem.md),
[`SUNION`](/content/commands/sunion.md) and others. For instance taking the intersection of two big sets
...
[Slow Log feature](/content/commands/slowlog.md).
...
of slow commands is the use of the [`KEYS`](/content/commands/keys.md) command in production environments.
[`KEYS`](/content/commands/keys.md), as documented in the Redis documentation, should only be used for
...
the [`SCAN`](/content/commands/scan.md), [`SSCAN`](/content/commands/sscan.md), [`HSCAN`](/content/commands/hscan.md) and [`ZSCAN`](/content/commands/zscan.md) commands for more information.
...
size. Data is obtained performing a BGSAVE and looking at the `latest_fork_usec` filed in the [`INFO`](/content/commands/info.md) command output.
...
This approach is needed in order to avoid using too much memory for keys that are already expired, and usually is absolutely harmless since it's strange that a big number of keys are going to expire in the same exact second, but it is not impossible that the user used [`EXPIREAT`](/content/commands/expireat.md) extensively with the same Unix time.
...
* The user enables the software watchdog using the [`CONFIG SET`](/content/commands/config-set.md) command.
