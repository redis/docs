...
This example runs the tests for the [`SET`](/commands/set) and [`LPUSH`](/commands/lpush) commands and uses quiet mode (see the `-q` switch).
...
Redis supports [pipelining]({{< relref "/develop/use/pipelining" >}}), so it is possible to send
...
example, a [`BGSAVE`](/commands/bgsave) operation in the background at the same time, will provide
