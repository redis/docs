...
An alternative to this scenario is to enable the Append Only File that will have this problem only when a log rewrite is performed using the [`BGREWRITEAOF`](/content/commands/bgrewriteaof.md) command.
...
For instance in the case of the [`SORT`](/content/commands/sort.md) command used together with the [`GET`](/content/commands/get.md) or `BY` options, it is not trivial to know beforehand what keys will be requested, so at least in the first implementation, `SORT BY/GET` resorts to the blocking VM implementation.
