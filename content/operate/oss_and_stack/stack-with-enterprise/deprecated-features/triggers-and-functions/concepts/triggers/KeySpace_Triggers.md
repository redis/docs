...
For a complete list of supported events, please refer to the [Redis keyspace notifications page](/content/develop/pubsub/keyspace-notifications.md#events-generated-by-different-commands).
...
* `callback`: The callback function to invoke, following the same rules of [Sync and Async invocation](/content/operate/oss_and_stack/stack-with-enterprise/deprecated-features/triggers-and-functions/concepts/Sync_Async.md). The callback will only be invoked on the primary shard.
