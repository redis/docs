...
| **Dynamic endpoints**      | Public and private endpoints that can be redirected to a Redis Cloud Pro database. (_Databases created before {{RELEASE DATE}} only_) We recommend using these endpoints for your applications. For more information, see [Applications that use legacy static endpoints]({{< relref "/operate/rc/databases/redirect-endpoints#applications-that-use-legacy-static-endpoints" >}}). |
