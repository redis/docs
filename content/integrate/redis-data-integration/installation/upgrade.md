...
{{< note >}}
Changing the pipeline processor and upgrading RDI are separate procedures.
If you want to move an existing pipeline to Flink, first follow
...
You can complete that procedure on RDI 1.19.0. Before upgrading to RDI 2.0.0,
set `processors.type` explicitly to `flink` or `classic` for every pipeline.
{{< /note >}}
