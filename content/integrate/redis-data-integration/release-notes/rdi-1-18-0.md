...
- **Optional AOF prerequisite check disablement**: For Helm installations, the `operator.prerequisiteChecks` section in the Helm values file lets you disable the AOF prerequisite check when the RDI database does not have AOF enabled. AOF should only be disabled after careful consideration, because disabling the check can lead to data loss in some failure scenarios. See [Can I use RDI without persistence enabled?](/content/integrate/redis-data-integration/faq.md#can-i-use-rdi-without-persistence-enabled).
