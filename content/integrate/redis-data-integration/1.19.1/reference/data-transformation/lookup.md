...
| **cache**           | `object`   | Cache the result of the argument expressions. See [`cache`]({{< relref "/integrate/redis-data-integration/1.19.1/reference/data-transformation/cache" >}}) for the property list. **Flink processor only.**<br/>                                                            | no       |
| **lookup_cache**    | `object`   | Cache the lookup results returned by Redis across batches, keyed by the resolved command arguments. Uses the same property list as [`cache`]({{< relref "/integrate/redis-data-integration/1.19.1/reference/data-transformation/cache" >}}). **Flink processor only.**<br/> | no       |
