...
Redis Functions is an API for managing code to be executed on the server. This feature, which became available in Redis 7, supersedes the use of [EVAL](/content/develop/programmability/eval-intro.md) in prior versions of Redis.

## Prologue (or, what's wrong with Eval Scripts?)

Prior versions of Redis made scripting available only via the [`EVAL`](/content/commands/eval.md) command, which allows a Lua script to be sent for execution by the server.
The core use cases for [Eval Scripts](/content/develop/programmability/eval-intro.md) is executing part of your application logic inside Redis, efficiently and atomically.
Such script can perform conditional updates across multiple keys, possibly combining several different data types.

Using [`EVAL`](/content/commands/eval.md) requires that the application sends the entire script for execution every time.
Because this results in network and script compilation overheads, Redis provides an optimization in the form of the [`EVALSHA`](/content/commands/evalsha.md) command. By first calling [`SCRIPT LOAD`](/content/commands/script-load.md) to obtain the script's SHA1, the application can invoke it repeatedly afterward with its digest alone.

By design, Redis only caches the loaded scripts.
That means that the script cache can become lost at any time, such as after calling [`SCRIPT FLUSH`](/content/commands/script-flush.md), after restarting the server, or when failing over to a replica.
...
1. Calling cached scripts within the context of a [transaction](/content/develop/using-commands/transactions.md) increases the probability of the transaction failing because of a missing script. Being more likely to fail makes using cached scripts as building blocks of workflows less attractive.
1. SHA1 digests are meaningless, making debugging the system extremely hard (e.g., in a [`MONITOR`](/content/commands/monitor.md) session).
1. When used naively, [`EVAL`](/content/commands/eval.md) promotes an anti-pattern in which scripts the client application renders verbatim scripts instead of responsibly using the [`KEYS` and `ARGV` Lua APIs](/content/develop/programmability/lua-api.md#runtime-globals).
...
Presently, as noted above, Redis ships with a single embedded [Lua 5.1](/content/develop/programmability/lua-api.md) engine.
...
with the only exception being the [Redis Lua scripts debugger](/content/develop/programmability/lua-debugging.md).
...
A function's execution blocks all server activities during its entire time, similarly to the semantics of [transactions](/content/develop/using-commands/transactions.md).
...
At this point, if you're unfamiliar with Lua in general and specifically in Redis, you may benefit from reviewing some of the examples in [Introduction to Eval Scripts](/content/develop/programmability/eval-intro.md) and [Lua API](/content/develop/programmability/lua-api.md) pages for a better grasp of the language.

Every Redis function belongs to a single library that's loaded to Redis.
Loading a library to the database is done with the [`FUNCTION LOAD`](/content/commands/function-load.md) command.
...
When the target execution engine handles the [`FUNCTION LOAD`](/content/commands/function-load.md) command, it registers the library's functions.
...
We can load our library and use [`FCALL`](/content/commands/fcall.md) to call the registered function:
...
Notice that the [`FUNCTION LOAD`](/content/commands/function-load.md) command returns the name of the loaded library, this name can later be used [`FUNCTION LIST`](/content/commands/function-list.md) and [`FUNCTION DELETE`](/content/commands/function-delete.md).

We've provided [`FCALL`](/content/commands/fcall.md) with two arguments: the function's registered name and the numeric value `0`. This numeric value indicates the number of key names that follow it (the same way [`EVAL`](/content/commands/eval.md) and [`EVALSHA`](/content/commands/evalsha.md) work).
...
We want an [`HSET`](/content/commands/hset.md)-like way to set and update fields in said Hashes and store the last modification time in a new field named `_last_modified_`.
We can implement a function to do all that.

Our function will call [`TIME`](/content/commands/time.md) to get the server's clock reading and update the target Hash with the new fields' values and the modification's timestamp.
...
We've added the `REPLACE` modifier to the call to [`FUNCTION LOAD`](/content/commands/function-load.md) to tell Redis that we want to overwrite the existing library definition.
...
In this case, we had invoked [`FCALL`](/content/commands/fcall.md) with _1_ as the number of key name arguments.
...
While all of the above should be straightforward, note that the `my_hgetall` also calls [`redis.setresp(3)`](/content/develop/programmability/lua-api.md#redis.setresp).
...
Once loaded, you can call the library's functions with [`FCALL`](/content/commands/fcall.md):
...
You can also get the library's details with the [`FUNCTION LIST`](/content/commands/function-list.md) command:
...
Upon success it returns `nil`, otherwise it returns an [error reply](/content/develop/programmability/lua-api.md#redis.error_reply).
...
For example, Redis needs to know that a certain function is read-only before permitting it to execute using [`FCALL_RO`](/content/commands/fcall_ro.md) on a read-only replica.
...
In our previous example, we defined two functions that only read data. We can try executing them using [`FCALL_RO`](/content/commands/fcall_ro.md) against a read-only replica.
...
1. Executing a function with [`FCALL`](/content/commands/fcall.md) against a read-only replica.
2. Using [`FCALL_RO`](/content/commands/fcall_ro.md) to execute a function.
...
To register a function with flags use the [named arguments](/content/develop/programmability/lua-api.md#redis.register_function_named_args) variant of `redis.register_function`.
...
Once we've replaced the library, Redis allows running both `my_hgetall` and `my_hlastmodified` with [`FCALL_RO`](/content/commands/fcall_ro.md) against a read-only replica:
...
For the complete documentation flags, please refer to [Script flags](/content/develop/programmability/lua-api.md#script_flags).
