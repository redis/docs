...
LDB uses the client-server model where the Redis server acts as a debugging server that communicates using [RESP](/content/develop/reference/protocol-spec.md). While `redis-cli` is the default debug client, any client can be used for debugging as long as it meets one of the following conditions:
