...
{{% alert title="Note" color="warning" %}}
By default, [`FT.SEARCH`](/commands/ft.search) returns only the first ten results. The [range query article]({{< relref "/develop/interact/search-and-query/query/range" >}}) explains to you how to scroll through the result set.
{{% /alert  %}}
