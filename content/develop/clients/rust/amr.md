...
letting the `redis-rs` client fetch and renew the authentication tokens for you automatically.
...
using the factory functions that `redis-rs` provides.


### `EntraIdCredentialsProvider` for a service principal

Use the `new_client_secret()` factory function to create a
