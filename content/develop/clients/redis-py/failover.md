...
see the main [Client-side geographic failover](/develop/clients/failover) page.
...
(see [Selecting a failover target](/develop/clients/failover#selecting-a-failover-target) for a full description of how
...
| Option | Description |
| --- | --- |
| `client_kwargs` | Keyword parameters to pass to the internal client constructor for this endpoint. Use it to specify the host, port, username, password, and other connection parameters (see [Connect to the server](/develop/clients/redis-py/connect) for more information). This is especially useful if you are using a custom client class (see [Client configuration](#client-configuration) below for more information). |
| `from_url` | Redis URL to connect to this endpoint, as an alternative to passing the host and port in `client_kwargs`. |
| `from_pool` | A `ConnectionPool` to supply the endpoint connection (see [Connect with a connection pool](/develop/clients/redis-py/connect#connect-with-a-connection-pool) for more information) |
...
(see [Detecting connection problems](/develop/clients/failover#detecting-connection-problems) for more information on how the
...
`RedisClient` connection (see [Retries](/develop/clients/redis-py/produsage#retries) for more information).
...
test (such as a [`PING`](/commands/ping) command) to determine if the
...
Redis Software [Active-Active](/operate/rs/databases/active-active)
deployments. It determines the health of the server by using the
[REST API](/operate/rs/references/rest-api) to check the
...
The example below
shows a simple custom strategy that sends a Redis [`ECHO`](/commands/echo)
...
`MultiDBClient` supports [Pub/Sub](/develop/pubsub)
...
in [Retries](/develop/clients/redis-py/produsage#retries)). However, if the client exhausts
...
(see [Timeouts](/develop/clients/redis-py/produsage#timeouts) for more information).
