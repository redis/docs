...
So `%R~llmcache:*` — the read-only shape the [Key permissions]() table below presents as sufficient for querying — is denied on the key, not the command. `+@read +@write` is unaffected, as is a cache with no TTL. See [Cache LLM Responses]({{< relref "how_to_guides/llmcache" >}}) for the refresh behaviour itself, including that `set_ttl()` starts adding TTLs to entries stored without one.
