...
A new timeseries can be created with the [`TS.CREATE`](/commands/ts.create) command; for example, to create a timeseries named `sensor1` run the following:
...
## Adding data points
For adding new data points to a timeseries we use the [`TS.ADD`](/commands/ts.add) command:
...
You can **append data points to multiple timeseries** at the same time with the [`TS.MADD`](/commands/ts.madd) command:
```
...
Data points between two timestamps (inclusive) can be deleted with the [`TS.DEL`](/commands/ts.del) command:
```
...
Or we can also use the [`TS.MGET`](/commands/ts.mget) command to get the last sample that matches the specific filter:
