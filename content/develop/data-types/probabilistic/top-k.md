...
The [`TOPK.LIST`](/commands/topk.list) command has a time complexity of `O(K)` so if `K` is small, there is no need to keep a separate set or sorted set of all the hashtags. You can query directly from the Top K itself. 
 ...
 * Use [`TOPK.LIST`](/commands/topk.list) to list the items entered thus far.
​
 * Use [`TOPK.QUERY`](/commands/topk.query) to see if an item is on the top K list. Just like [`TOPK.ADD`](/commands/topk.add) multiple items can be queried at the same time.
