...
{{< clients-example set="stream_tutorial" step="xadd" description="Foundational: Add entries to a stream using XADD with auto-generated IDs (creates new entries with field-value pairs)" prereq="true" >}}
...
{{< clients-example set="stream_tutorial" step="xrange" description="Foundational: Retrieve stream entries within a range of IDs using XRANGE when you need to access historical data" max_lines="10" needs_prereq="true" >}}
...
{{< clients-example set="stream_tutorial" step="xread_block" description="Use XREAD with BLOCK to wait for new entries when you need to consume messages as they arrive" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xadd_2" description="Foundational: Add a single entry to a stream with multiple field-value pairs using XADD" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xlen" description="Foundational: Get the total number of entries in a stream using XLEN" try_it="false" >}}
> XLEN race:france
(integer) 4
{{< /clients-example >}}
...
{{< clients-example set="stream_tutorial" step="xadd_id" description="Specify explicit stream entry IDs instead of auto-generated ones when you need to use external system IDs" difficulty="advanced" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xadd_bad_id" description="Understand ID validation - XADD rejects IDs that are not monotonically increasing" difficulty="advanced" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xadd_7" description="Use partial explicit IDs with XADD to specify milliseconds while letting Redis auto-generate the sequence number" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xrange_all" description="Foundational: Retrieve all entries in a stream using XRANGE with - and + special IDs" max_lines="10" try_it="false" >}}
> XRANGE race:france - +
1) 1) "1692632086370-0"
   2) 1) "rider"
      2) "Castilla"
...
{{< clients-example set="stream_tutorial" step="xrange_time" description="Query stream entries by time range using millisecond timestamps instead of full IDs" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xrange_step_1" description="Practical pattern: Paginate stream entries using XRANGE with COUNT to retrieve results in batches" difficulty="intermediate" max_lines="10" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xrange_step_2" description="Practical pattern: Continue pagination using exclusive range syntax with ( prefix to skip the last retrieved entry" difficulty="intermediate" max_lines="10" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xrange_empty" description="Practical pattern: Handle empty results when pagination reaches the end of the stream" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xrevrange" description="Foundational: Retrieve stream entries in reverse order using XREVRANGE when you need the most recent entries first" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xread" description="Foundational: Read entries from a stream starting from a specific ID using XREAD (non-blocking form)" max_lines="10" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xgroup_create" description="Foundational: Create a consumer group for a stream using XGROUP CREATE to enable coordinated message consumption" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xgroup_create_mkstream" description="Create a consumer group and stream atomically using XGROUP CREATE with MKSTREAM option" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xgroup_read" description="Foundational: Read new messages from a stream using a consumer group with XREADGROUP and the > special ID" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xgroup_read_id" description="Access pending message history using XREADGROUP with a specific ID to retrieve unacknowledged messages" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xack" description="Foundational: Acknowledge processed messages using XACK to mark them as handled by a consumer" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xgroup_read_bob" description="Practical pattern: Demonstrate consumer group load balancing where different consumers receive different messages from the same stream" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xpending" description="View pending message summary for a consumer group using XPENDING to monitor unacknowledged messages" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xpending_plus_minus" description="Get detailed pending message information including idle time and delivery count using XPENDING with range" difficulty="advanced" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xrange_pending" description="Retrieve the content of pending messages using XRANGE to inspect what needs to be processed" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xclaim" description="Claim pending messages from another consumer using XCLAIM when a consumer fails to process messages" difficulty="advanced" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xautoclaim" description="Practical pattern: Automatically claim idle pending messages using XAUTOCLAIM for simplified consumer failure recovery" difficulty="advanced" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xautoclaim_cursor" description="Continue automatic claiming using the cursor returned by XAUTOCLAIM to iterate through pending messages" difficulty="advanced" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xinfo" description="Get detailed stream information including length, encoding, and consumer groups using XINFO STREAM" difficulty="intermediate" max_lines="10" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xinfo_groups" description="List all consumer groups for a stream using XINFO GROUPS to see group status and pending message counts" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xinfo_consumers" description="Get detailed consumer information for a group using XINFO CONSUMERS to monitor individual consumer status" difficulty="advanced" max_lines="10" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="maxlen" description="Limit stream size using MAXLEN option with XADD to automatically evict old entries and maintain constant memory usage" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xtrim" description="Trim a stream to a maximum length using XTRIM MAXLEN to remove old entries" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xtrim2" description="Use approximate trimming with XTRIM MAXLEN ~ for more efficient memory management" difficulty="intermediate" try_it="false" >}}
...
{{< clients-example set="stream_tutorial" step="xdel" description="Delete specific entries from a stream by ID using XDEL for privacy or data cleanup purposes" difficulty="advanced" try_it="false" >}}
