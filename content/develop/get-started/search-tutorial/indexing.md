...
This is step 2 of the [Redis Search tutorial](/content/develop/get-started/search-tutorial/_index.md).
...
| `VECTOR` | Embeddings for similarity search (covered in the [last step](/content/develop/get-started/search-tutorial/vector-search.md)). | find products similar in meaning to a query |
...
Use [FT.CREATE](/content/commands/ft.create.md) to define the index. Because the data is JSON, each field is identified by a [JSONPath](/content/develop/data-types/json/path.md) expression, and `AS` gives it a short alias to use in queries:
...
You only create an index once. If you make a mistake, remove it with [FT.DROPINDEX](/content/commands/ft.dropindex.md) (this deletes the index, not your documents) and create it again.
...
> In the [Redis Insight Search workspace](/content/develop/tools/insight/search-workspace.md), your new `idx:catalog` index appears in the list of indexes. Selecting it shows the schema you just defined &mdash; the fields, their types, and their aliases &mdash; without having to read the raw `FT.INFO` output.

## Next steps

Your data is indexed. Continue to [searching and filtering](/content/develop/get-started/search-tutorial/search.md) to start asking questions of it.
