...
This guide shows you how to implement a distributed token bucket rate limiter using Redis and Lua scripts in Ruby with the [`redis-rb`](/content/develop/clients/ruby/_index.md) client library.
...
The **token bucket algorithm** is a popular rate limiting approach that allows bursts of traffic while maintaining an average rate limit over time. This guide covers the Ruby implementation using the [`redis-rb`](/content/develop/clients/ruby/_index.md) gem.
...
The Ruby implementation uses [`EVALSHA`](/content/commands/evalsha.md) for optimal performance. On first use, the Lua script is loaded into Redis with `SCRIPT LOAD`, and subsequent calls use the cached SHA1 hash. If the script is evicted from the cache, the module automatically falls back to [`EVAL`](/content/commands/eval.md) and reloads the script.
...
* [Ruby client](/content/develop/clients/ruby/_index.md) - Redis Ruby client documentation
