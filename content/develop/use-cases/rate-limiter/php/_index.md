...
This guide shows you how to implement a distributed token bucket rate limiter using Redis and Lua scripts in PHP with the [Predis](/content/develop/clients/php/_index.md) client library.
...
The PHP implementation uses [`EVALSHA`](/content/commands/evalsha.md) for optimal performance. On first use, the Lua script is loaded into Redis with `SCRIPT LOAD`, and subsequent calls use the cached SHA1 hash. If the script is evicted from the cache, the class automatically falls back to [`EVAL`](/content/commands/eval.md) and reloads the script.
...
* [PHP client](/content/develop/clients/php/_index.md) - Redis PHP client documentation
