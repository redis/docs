...
The **token bucket algorithm** is a popular rate limiting approach that allows bursts of traffic while maintaining an average rate limit over time. This guide covers the Node.js implementation using the [`node-redis`](/content/develop/clients/nodejs/_index.md) client library.
...
The Node.js implementation uses [`EVALSHA`](/content/commands/evalsha.md) for optimal performance. On first use, the Lua script is loaded into Redis with `SCRIPT LOAD`, and subsequent calls use the cached SHA1 hash. If the script is evicted from the cache, the module automatically falls back to [`EVAL`](/content/commands/eval.md) and reloads the script.
...
* [Node.js client](/content/develop/clients/nodejs/_index.md) - Redis Node.js client documentation
