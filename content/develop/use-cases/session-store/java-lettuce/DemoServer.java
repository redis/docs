...
/**
 * Redis Session Store Demo Server using Lettuce async commands.
 */
public class DemoServer {

    private static RedisClient redisClient;
    private static StatefulRedisConnection<String, String> connection;
    private static AsyncSessionStore sessionStore;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            sessionStore = new AsyncSessionStore(connection.async());
        } catch (Exception e) {
    ...
    static class SessionHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            Integer configuredTtl = sessionStore.getConfiguredTtl(sessionId).join();
            Long ttl = sessionStore.getTtl(sessionId).join();
    ...
    static class LoginHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            Map<String, String> session = enrichSession(sessionId);
            Integer configuredTtl = sessionStore.getConfiguredTtl(sessionId).join();
            Long remainingTtl = sessionStore.getTtl(sessionId).join();
    ...
    static class IncrementHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            Long pageViews = sessionStore.incrementField(sessionId, "page_views", 1).join();
            if (pageViews == null) {
    ...
    static class TtlHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            if (!sessionStore.setSessionTtl(sessionId, ttl).join()) {
                sendJson(exchange, 401, jsonPayload(false, null, null, null, null, null, "Session expired"), true);
                return;
            }
    ...
    static class LogoutHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            String sessionId = getSessionId(exchange);
            if (sessionId != null) {
                sessionStore.deleteSession(sessionId).join();
            }
    ...
    private static Map<String, String> enrichSession(String sessionId) {
        ...
        Map<String, String> session = sessionStore.getSession(sessionId, true).join();
        if (session == null) {
            return null;
        }

        session.put("ttl", String.valueOf(sessionStore.getTtl(sessionId).join()));
        Integer configuredTtl = sessionStore.getConfiguredTtl(sessionId).join();
    ...
    private static String htmlPage(String sessionId, Map<String, String> session) {
        return """
                ...
                <body>
                  <main>
                    <div class="pill">Lettuce async + Java HttpServer demo</div>
