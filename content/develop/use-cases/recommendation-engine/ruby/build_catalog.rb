...
#
# 1. Defines a small product catalogue inline so you can read and modify
#    it.
# 2. Runs the `informers` `embedding` pipeline (ONNX port of the
#    sentence-transformers `all-MiniLM-L6-v2` model) over the
#    `name + description` text of each product to produce a
...
# quickly without re-running the embedding model on every boot.
...
# pipeline: embed once on item-catalogue updates, write into Redis with
# `HSET`, and let the serving tier query without re-running the model.
...
# A small, hand-written catalogue. The fields here are the ones the
# Redis Search index expects (see recommender.rb): every field is either
# a TAG (exact-match filter), NUMERIC (range filter), TEXT (full-text),
...
# Build the string we actually embed for each product. Concatenating
# `name` and `description` gives the model both a short label and a
# longer descriptive context, which produces a more discriminating
# vector than embedding the name alone.
def embed_text_for(product)
  "#{product['name']}. #{product['description']}"
end

def main
  embedder = LocalEmbedder.new
  texts = CATALOG.map { |p| embed_text_for(p) }
  warn "Embedding #{texts.length} products with #{embedder.model_name}..."
  vectors = embedder.encode_many(texts)
  warn "Embeddings: count=#{vectors.length}, dim=#{vectors.first.length}, dtype=float32"

  out = {
    'model' => embedder.model_name,
    'dim' => embedder.dim,
    'products' => CATALOG.each_with_index.map do |product, i|
      # Pack the float32 vector as base64 so the JSON file stays
      # compact and the loader can hand the raw bytes straight to HSET
      # without re-converting them.
      product.merge('embedding_b64' => Base64.strict_encode64(vectors[i].pack('e*')))
    end,
  }

  out_path = File.join(__dir__, 'catalog.json')
  File.write(out_path, JSON.pretty_generate(out))
  warn "Wrote #{out['products'].length} products -> #{out_path}"
end
