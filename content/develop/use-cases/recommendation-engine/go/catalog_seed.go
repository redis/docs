...
// ``build_catalog.go``) runs the embedding model over each product's
// ``Name + Description`` and writes the result to ``catalog.json``.
//
// The fields here are the ones the Redis Search index expects (see
// ``recommender.go``): every field is either a TAG (exact-match
// filter), NUMERIC (range filter), TEXT (full-text), or the vector
// that gets written separately as a binary blob.

package recommendationengine

// Product is one row of the demo catalog. Serialised to JSON for
// ``catalog.json`` with snake_case field names so the file format is
// shared across the Python, Node.js, and Go ports.
type Product struct {
	ID          string  `json:"id"`
	...
	// EmbeddingB64 is populated by ``BuildCatalog`` after running the
	// model; it carries the float32 bytes of the embedding vector
...
// CatalogSeed is the 37-item demo catalog used to seed Redis.
...
// EmbedTextFor returns the string we actually embed for each product:
// ``Name. Description``. Concatenating the two gives the model both a
// short label and a longer descriptive context, which produces a more
// discriminating vector than embedding the name alone.
func EmbedTextFor(p Product) string {
