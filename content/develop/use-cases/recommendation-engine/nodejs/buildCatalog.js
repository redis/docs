 ...
 *
 *  1. Defines a small product catalog inline so you can read and modify
 *     it.
 *  2. Runs the @xenova/transformers model from ``embeddings.js`` over
 *     the ``name + description`` text of each product to produce a
...
// A small, hand-written catalog. The fields here are the ones the
// Redis Search index expects (see recommender.js): every field is either
...
/**
 * Build the string we actually embed for each product.
 *
 ...
 */
function embedTextFor(product) {
  return `${product.name}. ${product.description}`;
}

async function main() {
  const embedder = new LocalEmbedder();
  const texts = CATALOG.map(embedTextFor);
  console.log(
    `Embedding ${texts.length} products with ${embedder.modelName}...`,
  );
  const vectors = await embedder.encodeMany(texts);
  const dim = vectors[0].length;
  console.log(`Embeddings: shape=[${vectors.length}, ${dim}], dtype=float32`);

  const out = {
    model: embedder.modelName,
    dim,
    products: CATALOG.map((product, i) => ({
      ...product,
  ...
  console.log(`Wrote ${out.products.length} products -> ${outPath}`);
}
