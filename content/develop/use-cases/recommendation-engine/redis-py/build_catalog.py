...
2. Runs the sentence-transformers model from ``embeddings.py`` over the
...
try:
    ...
    print(f"Error: {exc}")
    print(
        "Make sure 'sentence-transformers' and 'numpy' are installed:\n"
...
# Redis Search index expects (see recommender.py): every field is either
...
def embed_text_for(product: dict) -> str:
...
def main() -> None:
    ...
    texts = [embed_text_for(p) for p in CATALOG]
    print(f"Embedding {len(texts)} products with {embedder.model_name}...")
    ...
    print(f"Wrote {len(out['products'])} products -> {out_path}")
