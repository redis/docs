// HTML template for the demo UI.
//
// Copied verbatim from the Python reference at
// content/develop/use-cases/recommendation-engine/redis-py/demo_server.py
// (the HTML_TEMPLATE constant). Only the "pill" text at the top has
// been swapped to identify this port. The JS in this template depends
// on the exact JSON field names that DemoServer.cs returns from the
// /state, /search, /click, /reset-user, /reset-index, and
// /refresh-embedding endpoints, so the field names must stay aligned
// across the Python, Node.js, Go, and .NET ports.

namespace RecommendationDemo;

internal static class DemoHtml
{
    public static string Render(string indexName, string userKey, int topK)
    {
        return Template
            .Replace("__INDEX_NAME__", indexName)
            .Replace("__USER_KEY__", userKey)
            .Replace("__TOPK__", topK.ToString());
    }

    // Raw string literal (C# 11) lets the template stay readable
...
<body>
  <main>
    <div class="pill">NRedisStack + SmartComponents.LocalEmbeddings + System.Net.HttpListener</div>
