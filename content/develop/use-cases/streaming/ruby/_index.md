...
1. The application calls `stream.produce(event_type, payload)` which runs [`XADD`](/content/commands/xadd.md) with an approximate [`MAXLEN ~`](/content/commands/xadd.md) cap. Redis assigns an auto-generated time-ordered ID.
...
See the [`redis-rb` documentation](/content/develop/clients/ruby/_index.md) for the full client reference, and the [Streams overview](/content/develop/data-types/streams/_index.md) for the deeper conceptual model — consumer groups, the PEL, claim semantics, capped streams, and the differences with Kafka partitions.
