...
# at startup.
#
# The seed list mixes `semantic` memories (long-lived preferences and
# facts) with `episodic` memories (snapshots of past sessions),
# matching what every other agent-memory port seeds so the
# implementations behave identically regardless of which one you
# point at a given Redis instance.

require_relative 'embeddings'
require_relative 'long_term_memory'
...
module AgentMemory
  module SeedMemory
    SEED_MEMORIES = [
      ...
      {
        text: 'The user dislikes dark mode and prefers a high-contrast ' \
              'light theme in editors and dashboards.',
        kind: 'semantic'
      },
      {
        text: 'The user is allergic to peanuts; any restaurant suggestion ' \
              'must avoid dishes that commonly contain them.',
        kind: 'semantic'
      },
      {
        text: 'Last Tuesday the user asked the agent to draft a postmortem ' \
      ...
      {
        text: 'In a previous session the user asked for help debugging a ' \
              'flaky test in the inventory service. The fix turned out ' \
              'to be a race condition in the warehouse webhook handler.',
        kind: 'episodic'
      },
      {
        text: 'Two weeks ago the user mentioned they were planning to ' \
    ...
    # count).
    def self.seed(memory, embedder,
                  user: 'default', namespace: 'default',
                  source_thread: 'seed')
      texts = SEED_MEMORIES.map { |m| m[:text] }
      vectors = embedder.encode_many(texts)
      written = 0
      SEED_MEMORIES.each_with_index do |entry, i|
        ...
        written += 1 unless result.deduped
      end
      written
    end
  end
