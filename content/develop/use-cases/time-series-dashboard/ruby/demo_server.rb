...
require "redis"
require "webrick"
require_relative "sensor_simulator"
require_relative "timeseries_store"
...
begin
  ...
  warn "Failed to connect to Redis: #{error.message}"
  warn "Make sure Redis is running at #{redis_host}:#{redis_port}"
  exit 1
end

store = RedisTimeSeriesStore.new(redis: redis_client, sensors: SensorSimulator::SENSORS)
begin
  store.ensure_schema
rescue Redis::CommandError => error
  warn "TimeSeries setup failed: #{error.message}"
  warn "Make sure your Redis deployment includes time series support."
  exit 1
end
...
Thread.new do
  loop do
    samples = simulator.next_samples
    redis_mutex.synchronize do
      begin
        store.add_samples(samples)
      rescue StandardError => error
        warn "Failed to add samples: #{error.message}"
      end
    end
    sleep(RedisTimeSeriesStore::SAMPLE_INTERVAL_MS / 1000.0)
  end
end
    ...
    <body>
      <main>
        <section class="intro">
          ...
          <p>Three sensors write new readings to Redis every <code>#{RedisTimeSeriesStore::SAMPLE_INTERVAL_MS}ms</code>. Each graph shows only the most recent <code>#{RedisTimeSeriesStore::WINDOW_MS / 1000}s</code>, so old samples disappear as Redis retention expires them. The buckets underneath each graph summarize the same data in <code>#{RedisTimeSeriesStore::BUCKET_MS / 1000}s</code> windows using min, max, and average aggregation.</p>
          <div class="metrics">
            <span class="pill">Commands in play: <code>TS.MADD</code>, <code>TS.RANGE</code></span>
            <span class="pill">Retention: <code>#{RedisTimeSeriesStore::RETENTION_MS / 1000}s</code></span>
            <span class="pill">Sample interval: <code>#{RedisTimeSeriesStore::SAMPLE_INTERVAL_MS}ms</code></span>
            <span class="pill">Bucket width: <code>#{RedisTimeSeriesStore::BUCKET_MS / 1000}s</code></span>
          </div>
        </section>
...
puts "Dashboard running at http://localhost:#{port}"
puts "Writing simulated samples to Redis at #{redis_host}:#{redis_port}"
server.start
