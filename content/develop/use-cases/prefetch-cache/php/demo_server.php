 ...
 *
 * Visit http://127.0.0.1:8788 to watch a prefetch cache in action: the
 * server bulk-loads every primary record into Redis on first request,
 * spawns a long-running sync-worker process that applies primary
 ...
 *
 * Because the PHP built-in dev server runs each HTTP request in a
 * fresh process, the sync worker can't be hosted inside this server.
 * Instead, the first request spawns one `php sync_worker.php` OS
 * process via proc_open and tracks its PID in Redis under
 * demo:sync:pid. Pause/resume coordination on /clear and /reprefetch
 * is also Redis-backed (demo:sync:paused / demo:sync:idle).
 *
 * CLI / env config (env vars or `php -d ...`):
 *     PREFETCH_REDIS_HOST     default 127.0.0.1
 *     PREFETCH_REDIS_PORT     default 6379
 *     PREFETCH_CACHE_PREFIX   default cache:category:
 *     PREFETCH_TTL_SECONDS    default 3600
 *     PREFETCH_LATENCY_MS     default 80 (simulated primary read latency)
 */
...
$redisHost = getenv('PREFETCH_REDIS_HOST') ?: '127.0.0.1';
$redisPort = (int) (getenv('PREFETCH_REDIS_PORT') ?: 6379);
$cachePrefix = getenv('PREFETCH_CACHE_PREFIX') ?: 'cache:category:';
$ttlSeconds = (int) (getenv('PREFETCH_TTL_SECONDS') ?: 3600);
$latencyMs = (int) (getenv('PREFETCH_LATENCY_MS') ?: 80);
...
$cache = new PrefetchCache($redis, $cachePrefix, $ttlSeconds);
$primary = new MockPrimaryStore($redis, $latencyMs);
$supervisor = new SyncWorkerSupervisor(
    $redis,
    __DIR__ . '/sync_worker.php',
    [
        'redis-host' => $redisHost,
        'redis-port' => (string) $redisPort,
        'cache-prefix' => $cachePrefix,
        'ttl-seconds' => (string) $ttlSeconds,
        'primary-latency-ms' => (string) $latencyMs,
    ]
);

// First-request bootstrap: seed the primary, prefetch the cache, spawn
// the sync worker. Idempotent — the next request finds the bootstrap
// flag and the worker PID already there and skips everything.
$bootstrapKey = 'demo:bootstrap:' . $cachePrefix;
$bootstrapDone = ((string) $redis->get($bootstrapKey)) === '1';
$primary->seedIfEmpty();
if (!$bootstrapDone || !$supervisor->running()) {
    if (!$bootstrapDone) {
        $cache->clear();
        $cache->bulkLoad($primary->listRecords());
    }
    $supervisor->start();
    $redis->set($bootstrapKey, '1');
}
...
if ($method === 'GET' && ($path === '/' || $path === '/index.html')) {
    send_html(render_page($cache->getTtlSeconds()));
    return;
}
...
if ($method === 'GET' && $path === '/read') {
    ...
    [$record, $hit, $redisMs] = $cache->get($entityId);
    send_json([
        'id' => $entityId,
        'record' => $record,
        'hit' => $hit,
        'redis_latency_ms' => round($redisMs, 2),
        'ttl_remaining' => $cache->ttlRemaining($entityId),
...
if ($method === 'POST' && $path === '/update') {
    ...
    if (!$primary->updateField($entityId, $field, $value)) {
        send_json(['error' => "Unknown category '{$entityId}'."], 404);
        return;
    }
    send_json([
        'id' => $entityId,
...
if ($method === 'POST' && $path === '/add') {
    $params = read_form_data();
    $entityId = trim((string) ($params['id'] ?? ''));
    $name = trim((string) ($params['name'] ?? ''));
    if ($entityId === '' || $name === '') {
        send_json(['error' => "Missing 'id' or 'name'."], 400);
        return;
    }
    $record = [
        'id' => $entityId,
        'name' => $name,
        'display_order' => (string) ($params['display_order'] ?? '99') ?: '99',
        'featured' => (string) ($params['featured'] ?? 'false') ?: 'false',
        'parent_id' => (string) ($params['parent_id'] ?? '') ?: '',
    ];
    if (!$primary->addRecord($record)) {
        send_json(['error' => "Category '{$entityId}' already exists."], 409);
        return;
    }
    send_json([
        'id' => $entityId,
...
if ($method === 'POST' && $path === '/delete') {
    ...
    if (!$primary->deleteRecord($entityId)) {
        send_json(['error' => "Unknown category '{$entityId}'."], 404);
        return;
    }
    send_json(['id' => $entityId, 'stats' => build_stats($cache, $primary)]);
    return;
}
...
if ($method === 'POST' && $path === '/invalidate') {
    ...
    $deleted = $cache->invalidate($entityId);
    send_json([
        'id' => $entityId,
        'deleted' => $deleted,
        'stats' => build_stats($cache, $primary),
...
if ($method === 'POST' && $path === '/clear') {
    ...
    // and DEL. Queued events accumulate in demo:primary:changes and
    // apply in order after resume().
    $supervisor->pause();
    try {
        $deleted = $cache->clear();
    } finally {
...
if ($method === 'POST' && $path === '/reprefetch') {
...
function build_stats(PrefetchCache $cache, MockPrimaryStore $primary): array
{
    $stats = $cache->stats();
    $stats['primary_reads_total'] = $primary->reads();
    $stats['primary_read_latency_ms'] = $primary->getReadLatencyMs();
    return $stats;
}
...
function render_page(int $ttlSeconds): string
{
    $cacheTtl = (string) $ttlSeconds;
    return <<<HTML
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  <main>
    <div class="pill">Predis + php -S dev server</div>
    ...
    <p class="lede">
      ...
      safety-net TTL ({$cacheTtl} s) is refreshed on every add or update
