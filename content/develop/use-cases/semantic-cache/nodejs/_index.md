...
The embedder is [`@xenova/transformers`](https://www.npmjs.com/package/@xenova/transformers) running the ONNX-exported [`Xenova/all-MiniLM-L6-v2`](https://huggingface.co/Xenova/all-MiniLM-L6-v2) model, which is the same encoder the [Python example](/content/develop/use-cases/semantic-cache/redis-py/_index.md) uses. Embeddings produced by the two implementations are semantically equivalent — paraphrase distances differ only at the fourth decimal place — so a cache populated by one demo can be queried by the other against the same Redis instance.
...
3. If the distance is at or below the threshold, the cache returns a hit (`{ kind: 'hit', ... }`) containing the cached response. The helper also runs an [`HINCRBY`](/content/commands/hincrby.md) on `hit_count` and an [`EXPIRE`](/content/commands/expire.md) refresh inside a [`MULTI/EXEC`](/content/commands/multi.md), so a frequently used answer keeps its TTL and the demo UI can see which entries are load-bearing.
