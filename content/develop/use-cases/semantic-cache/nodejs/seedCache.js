...
// at startup.
//
// The seed list mirrors the keyword table in `mockLlm.js` but stores
...
export async function seed(cache, embedder, {
  tenant = 'acme',
  locale = 'en',
  modelVersion = 'gpt-4.5-2026',
} = {}) {
  const prompts = SEED_ENTRIES.map(e => e.prompt);
  const vectors = await embedder.encodeMany(prompts);
