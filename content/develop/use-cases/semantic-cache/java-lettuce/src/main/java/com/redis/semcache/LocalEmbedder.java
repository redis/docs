...
public final class LocalEmbedder implements AutoCloseable {
    ...
    /**
     * Scale {@code vector} to unit length in place. DJL's default
     * translator already returns near-unit vectors for this model,
     * so the multiplier sits right on top of {@code 1.0} — but
     * re-normalising explicitly insulates the demo from any future
     * change in the translator's defaults, and a vector that has
     * drifted by even one part in a million would otherwise leak
     * into the cosine distance the demo prints to the UI.
     */
