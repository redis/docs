...
# Go, and Jedis siblings use. Vectors are L2-normalised so a Redis
# Search index declared with `DISTANCE_METRIC COSINE` returns scores
# that are directly comparable across entries.
#
# Embeddings are numerically very close to the PyTorch reference
# (matches the Node.js Xenova ONNX path to ~0.01 in cosine distance);
# the model is downloaded into the local Hugging Face cache on the
# first call and every later call runs offline.
...
module SemCache
  ...
  # cache's expected vector dimension before doing any inserts;
  # RedisSemanticCache also checks length on every put / lookup, so a
