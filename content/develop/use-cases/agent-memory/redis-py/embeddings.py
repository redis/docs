...
Vectors are L2-normalized on output so a Redis Search index declared
...
class LocalEmbedder:
    """Encode short strings into normalized float32 vectors.
    ...
    lifetime of the process so each turn embeds the new memory and
    the recall query with the same model in two encode calls.
    """
