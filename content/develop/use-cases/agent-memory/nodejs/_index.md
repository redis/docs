...
5.  Open <http://localhost:8089> and try some turns:
    ...
    *  **"I prefer concise answers without filler phrases."** — paraphrase of a
       seed; you should see the write **deduped** onto the existing memory at a
       cosine distance around 0.18 (the ONNX-exported model runs slightly
       different arithmetic from the PyTorch one, so paraphrase distances
       sit a hair higher than in the Python demo), with `hit_count` ticking
       up in the memories table.
