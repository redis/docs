...
// the fourth decimal place, so the distance bands quoted in the
// Python walkthrough carry over to this demo without re-calibration).
//
...
export class LocalEmbedder {
  ...
  static async create(modelName = DEFAULT_MODEL) {
    ...
    // callers can compare against the index's expected vectorDim
    // before doing any inserts. LongTermMemory also checks length on
    // every remember / recall, so a model swap that produces
