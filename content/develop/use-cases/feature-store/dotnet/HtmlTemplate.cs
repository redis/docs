...
<head>
  ...
  <title>Redis Feature Store Demo (.NET)</title>
  <style>
    :root {
      --bg: #eef3f1;
      --panel: #ffffff;
...
<body>
    ...
    <div class="grid">
      ...
      <section class="panel">
        ...
        <p>Calls <code>HSET</code> + <code>EXPIRE</code> for each user
          through one <code>IBatch</code> — the whole batch ships in
          one round trip.</p>
      ...
      <section class="panel">
        ...
        <p>Pipelined <code>HMGET</code> across N random users via
          <code>IBatch</code>. One network round trip for the whole
          batch.</p>
  ...
  <script>
    ...
    function renderStore(state) {
      ...
      storeView.innerHTML = `
        <dl>
          ...
          <dt>Batch writes</dt><dd>${stats.batchWritesTotal ?? 0}</dd>
          <dt>Streaming writes</dt><dd>${stats.streamingWritesTotal ?? 0}</dd>
          <dt>Reads</dt><dd>${stats.readsTotal ?? 0}</dd>
          <dt>Fields returned</dt><dd>${stats.readFieldsTotal ?? 0}</dd>
        </dl>`;
    }
    function renderWorker(state) {
      ...
      workerView.innerHTML = `<p>${badge} <span class="mono">ticks=${w.tickCount ?? 0} writes=${w.writesCount ?? 0}</span></p>`;
    }
