...
This guide shows you how to implement Redis-backed leaderboards in Rust with the [`redis-rs`](/content/develop/clients/rust/_index.md) client library. It includes both synchronous and asynchronous APIs, uses a sorted set to store rank order, stores per-user metadata in Redis hashes, and includes a small local web server so you can explore the leaderboard interactively in your browser.
