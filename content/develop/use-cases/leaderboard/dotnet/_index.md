...
This guide shows you how to implement a Redis-backed leaderboard in C# with the [`StackExchange.Redis`](/content/develop/clients/dotnet/_index.md) client library. It uses a sorted set to store rank order, Redis hashes to store per-user metadata, and a small local web server so you can explore the leaderboard interactively in your browser.
