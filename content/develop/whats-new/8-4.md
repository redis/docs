...
- [`DELEX`](/content/commands/delex.md): Delete a key only if its value matches a specified condition.
- [`DIGEST`](/content/commands/digest.md): Generate cryptographic digests of key values for integrity verification.

The [`SET`](/content/commands/set.md) command now supports atomic compare-and-set and compare-and-delete operations through new extensions, enabling you to implement lock-free data structures and optimistic concurrency control patterns.

### Multi-key expiration management

The new [`MSETEX`](/content/commands/msetex.md) command allows you to set multiple keys and update their expiration times in a single atomic operation. This simplifies batch operations where you need to maintain consistent TTL values across related keys.

### Advanced stream processing

Redis 8.4 enhances stream processing with the `CLAIM min-idle-time` option for [`XREADGROUP`](/content/commands/xreadgroup.md). This feature enables consumer groups to automatically claim and process both idle pending entries and new incoming entries in a single operation, simplifying consumer group management and improving processing efficiency.
