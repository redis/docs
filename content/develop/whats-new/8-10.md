...
Redis 8.10 introduces compact hashes (#15364), a new hash encoding that reduces memory usage by storing hash field names just once for keys that share a schema. The new [`HIMPORT`](/content/commands/himport.md) command (#15364) provides high-throughput bulk insertion of compact hashes.

### BACKUP command

The new [`BACKUP`](/content/commands/backup.md) command (#15441) adds a node-side implementation for backup and restore based on multi-part AOF (MP-AOF).

### List multi-element moves

The new [`LMOVEM`](/content/commands/lmovem.md) and [`BLMOVEM`](/content/commands/blmovem.md) commands (#15405) move multiple elements between lists in a single operation.

### Set cardinality commands

The new [`SUNIONCARD`](/content/commands/sunioncard.md) command (#14893) returns the cardinality of the union of multiple sets, and the new [`SDIFFCARD`](/content/commands/sdiffcard.md) command (#15278) returns the cardinality of the difference between sets, without materializing the result.

### Streams reply caps

[`XREAD`](/content/commands/xread.md) and [`XREADGROUP`](/content/commands/xreadgroup.md) accept new `MAXCOUNT` and `MAXSIZE` arguments (#15282) to cap the cumulative number of reply entries and the cumulative reply size.
...
[`SLOWLOG GET`](/content/commands/slowlog-get.md) adds a new reply argument (#15347): the total argument count for the logged command.

### FT.ALIASLIST

The new [`FT.ALIASLIST`](/content/commands/ft.aliaslist.md) command (#Q9626) returns all aliases for an index (RED-197340).
...
[`FT.AGGREGATE`](/content/commands/ft.aggregate.md) gains a new `COLLECT` reducer (#Q9291) that separates remote and local reducer invocations (RED-177887).

### Stricter query timeout enforcement

[`FT.SEARCH`](/content/commands/ft.search.md), [`FT.AGGREGATE`](/content/commands/ft.aggregate.md), and [`FT.HYBRID`](/content/commands/ft.hybrid.md) now enforce the query `TIMEOUT` more strictly (#Q8169, #Q8236, #Q9234, #Q9443, #Q9861).
...
Queries executed on the main thread (that is, when `search-workers` is 0) are capped by [`search-_max-foreground-timeout-limit`](/content/develop/ai/search-and-query/administration/configuration.md#search-_max-foreground-timeout-limit) (default `60000` ms).
...
- The new [`TS.NRANGE`](/content/commands/ts.nrange.md) and [`TS.NREVRANGE`](/content/commands/ts.nrevrange.md) commands (#T2052) query a range across multiple time series and group results by timestamp (RED-149232).
- The new [`TS.READ`](/content/commands/ts.read.md) command (#T2054) performs an optionally blocking read (RED-132421).
- The new [`TS.QUERYLABELS`](/content/commands/ts.querylabels.md) command (#T2090) returns a list of labels and label-values (RED-132355).
- [`TS.MRANGE`](/content/commands/ts.mrange.md) and [`TS.MREVRANGE`](/content/commands/ts.mrevrange.md) accept a new `EXCLUDEEMPTY` argument (#T2072) to exclude series with no reported samples (RED-132536).
