...
The new [`INCREX`](/content/commands/increx.md) command (#15045) is a window counter rate limiter that combines `INCR`, `INCRBY`, `INCRBYFLOAT`, bounds, and expiration into a single atomic operation. It simplifies common rate-limiting patterns by eliminating the need for multi-command transactions.

### XNACK streams command

The new [`XNACK`](/content/commands/xnack.md) command allows stream consumers to explicitly release pending messages, providing finer-grained control over consumer group pending entry lists.

### Sorted set COUNT aggregator

[`ZUNION`](/content/commands/zunion.md), [`ZINTER`](/content/commands/zinter.md), [`ZUNIONSTORE`](/content/commands/zunionstore.md), and [`ZINTERSTORE`](/content/commands/zinterstore.md) support a new `COUNT` aggregator for combining sorted sets.

### JSON.SET FPHA argument

[`JSON.SET`](/content/commands/json.set.md) accepts a new `FPHA` argument that specifies the floating-point type for homogeneous FP arrays.

### Time series multiple aggregators

[`TS.RANGE`](/content/commands/ts.range.md), [`TS.REVRANGE`](/content/commands/ts.revrange.md), [`TS.MRANGE`](/content/commands/ts.mrange.md), and [`TS.MREVRANGE`](/content/commands/ts.mrange.md) now support multiple aggregators in a single command.

### FT.HYBRID candidates per shard

The [`FT.HYBRID`](/content/commands/ft.hybrid.md) `KNN` clause accepts a new argument to request fewer candidates per shard, reducing overhead in cluster mode.

### FT.PROFILE HYBRID

[`FT.PROFILE`](/content/commands/ft.profile.md) adds profiling support for [`FT.HYBRID`](/content/commands/ft.hybrid.md) queries.

### In-group sorting reducer

A new reducer for [`FT.AGGREGATE`](/content/commands/ft.aggregate.md) allows you to unwind grouped documents after `GROUPBY` and sort them within each group, enabling more sophisticated aggregation pipelines.
