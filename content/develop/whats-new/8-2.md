...
- [`XDELEX`](/content/commands/xdelex.md): Delete entries from streams with advanced consumer group handling options.
- [`XACKDEL`](/content/commands/xackdel.md): Acknowledge and delete stream entries in a single atomic operation.
...
The [`XADD`](/content/commands/xadd.md) and [`XTRIM`](/content/commands/xtrim.md) commands have also been extended with these same options, providing consistent behavior across all commands that remove stream entries.

### Cluster monitoring improvements

The new [`CLUSTER SLOT-STATS`](/content/commands/cluster-slot-stats.md) command provides detailed per-slot usage metrics including:
- Key count per slot
...
Redis 8.2 extends the [`BITOP`](/content/commands/bitop.md) command with new operators that provide more sophisticated bit manipulation capabilities:
