...
## Details
The assigned name is displayed in the output of [`CLIENT LIST`](/content/commands/client-list.md) so that it is possible to identify the client that performed a given connection.
...
There is no limit to the length of the name that can be assigned if not the usual limits of the Redis string type (512 MB). However it is not possible to use spaces in the connection name as this would violate the format of the [`CLIENT LIST`](/content/commands/client-list.md) reply.
...
The connection name can be inspected using [`CLIENT GETNAME`](/content/commands/client-getname.md).
