...
title: MEMORY STATS
---
The `MEMORY STATS` command returns an [Array reply](/content/develop/reference/protocol-spec.md#arrays) about the memory usage of the
server.

The information about memory usage is provided as metrics and their respective
values. The following metrics are reported:

*   `peak.allocated`: Peak memory consumed by Redis in bytes (see [`INFO`](/content/commands/info.md)'s
     `used_memory_peak`)
*   `total.allocated`: Total number of bytes allocated by Redis using its
     allocator (see [`INFO`](/content/commands/info.md)'s `used_memory`)
*   `startup.allocated`: Initial amount of memory consumed by Redis at startup
     in bytes (see [`INFO`](/content/commands/info.md)'s `used_memory_startup`)
*   `replication.backlog`: Size in bytes of the replication backlog (see
     [`INFO`](/content/commands/info.md)'s `repl_backlog_active`)
...
*   `clients.normal`: The total size in bytes of all clients overheads (output
     and query buffers, connection contexts)
*   `cluster.links`: Memory usage by cluster links (Added in Redis 7.0, see [`INFO`](/content/commands/info.md)'s `mem_cluster_links`).
...
*   `functions.caches`: the summed size in bytes of the overheads of the Function scripts'
     caches
*   `hash.templates`: Total memory in bytes used by all compact hash templates (see [`INFO`](/content/commands/info.md)'s `used_memory_hash_templates`). Added in Redis 8.10.
*   `dbXXX`: For each of the server's databases, the overheads of the main,
     expiry, and [blessed-key](/content/commands/bless) dictionaries
...
*   `overhead.total`: The sum of all overheads, i.e. `startup.allocated`,
     ...
     Redis keyspace (see [`INFO`](/content/commands/info.md)'s `used_memory_overhead`)
...
*   `dataset.bytes`: The size in bytes of the dataset, i.e. `overhead.total`
     subtracted from `total.allocated` (see [`INFO`](/content/commands/info.md)'s `used_memory_dataset`)
...
*   `peak.percentage`: The percentage of `total.allocated` out of
     `peak.allocated`
*   `allocator.allocated`: See [`INFO`](/content/commands/info.md)'s `allocator_allocated`
*   `allocator.active`: See [`INFO`](/content/commands/info.md)'s `allocator_active`
*   `allocator.resident`: See [`INFO`](/content/commands/info.md)'s `allocator_resident`
*   `allocator.muzzy`: See [`INFO`](/content/commands/info.md)'s `allocator_muzzy`
*   `allocator-fragmentation.ratio`: See [`INFO`](/content/commands/info.md)'s `allocator_frag_ratio`
*   `allocator-fragmentation.bytes`: See [`INFO`](/content/commands/info.md)'s `allocator_frag_bytes`
*   `allocator-rss.ratio`: See [`INFO`](/content/commands/info.md)'s `allocator_rss_ratio`
*   `allocator-rss.bytes`: See [`INFO`](/content/commands/info.md)'s `allocator_rss_bytes`
*   `rss-overhead.ratio`: See [`INFO`](/content/commands/info.md)'s `rss_overhead_ratio`
*   `rss-overhead.bytes`: See [`INFO`](/content/commands/info.md)'s `rss_overhead_bytes`
*   `fragmentation`: See [`INFO`](/content/commands/info.md)'s `mem_fragmentation_ratio`
*   `fragmentation.bytes`: See [`INFO`](/content/commands/info.md)'s `mem_fragmentation_bytes`
