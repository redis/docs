...
normally as a side effect of calling [`XREADGROUP`](/content/commands/xreadgroup.md), or when a consumer took
ownership of a message calling [`XCLAIM`](/content/commands/xclaim.md). The pending message was delivered to
some consumer but the server is yet not sure it was processed at least once.
So new calls to [`XREADGROUP`](/content/commands/xreadgroup.md) to grab the messages history for a consumer
(for instance using an ID of 0), will return such message.
Similarly the pending message will be listed by the [`XPENDING`](/content/commands/xpending.md) command,
