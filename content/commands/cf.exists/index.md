...
This command is similar to [`CF.MEXISTS`](/commands/cf.mexists), except that only one item can be checked.
...
- [Integer reply](/docs/reference/protocol-spec#integers), where `1` means that, with high probability, `item` had already been added to the filter, and `0` means that `key` does not exist or that `item` had not been added to the filter. See note in [`CF.DEL`](/commands/cf.del).
