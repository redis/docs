...
It is the blocking version of [`LPOP`](/content/commands/lpop.md) because it blocks the connection when there
...
client performs an [`LPUSH`](/content/commands/lpush.md) or [`RPUSH`](/content/commands/rpush.md) operation against one of the keys.
...
* After an [`EXEC`](/content/commands/exec.md) of a [`MULTI`](/content/commands/multi.md) block with multiple push operations against the same list.
...
If the above condition happens using a Redis 2.6 server or greater, Client **A** will be served with the `c` element, because after the [`LPUSH`](/content/commands/lpush.md) command the list contains `c,b,a`, so taking an element from the left means to return `c`.
...
Using `BLPOP` inside a [`MULTI`](/content/commands/multi.md) / [`EXEC`](/content/commands/exec.md) block does not make a lot of sense
as it would require blocking the entire server in order to execute the block
atomically, which in turn does not allow other clients to perform a push
operation. For this reason the behavior of `BLPOP` inside [`MULTI`](/content/commands/multi.md) / [`EXEC`](/content/commands/exec.md) when the list is empty is to return a `nil` multi-bulk reply, which is the same
...
[`MULTI`](/content/commands/multi.md) / [`EXEC`](/content/commands/exec.md) block...
...
This can be a problem with some application where we want a more reliable messaging system. When this is the case, please check the [`BRPOPLPUSH`](/content/commands/brpoplpush.md) command, that is a variant of `BLPOP` that adds the returned element to a target list before returning it to the client.
...
This would require a blocking version of [`SPOP`](/content/commands/spop.md) that is not available, but using
