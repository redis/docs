...
- If a key already exists, you get a Redis error reply, `TSDB: key already exists`. You can check for the existence of a key with the [`EXISTS`](/content/commands/exists.md) command.
- Other commands that also create a new time series when called with a key that does not exist are [`TS.ADD`](/content/commands/ts.add.md), [`TS.INCRBY`](/content/commands/ts.incrby.md), and [`TS.DECRBY`](/content/commands/ts.decrby.md).
</note>
...
is maximum age for samples compared to the highest reported timestamp, in milliseconds. Samples are expired based solely on the difference between their timestamp and the timestamps passed to subsequent [`TS.ADD`](/content/commands/ts.add.md), [`TS.MADD`](/content/commands/ts.madd.md), [`TS.INCRBY`](/content/commands/ts.incrby.md), and [`TS.DECRBY`](/content/commands/ts.decrby.md) calls with this key.

When set to 0, samples never expire. When not specified, the option is set to the global [RETENTION_POLICY](/content/develop/data-types/timeseries/configuration.md#retention_policy) configuration of the database, which by default is 0.
</details>
...
is initial allocation size, in bytes, for the data part of each new chunk. Actual chunks may consume more memory. Changing chunkSize (using [`TS.ALTER`](/content/commands/ts.alter.md)) does not affect existing chunks.

Must be a multiple of 8 in the range [48 .. 1048576]. When not specified, it is set to the global [CHUNK_SIZE_BYTES](/content/develop/data-types/timeseries/configuration.md#chunk_size_bytes) configuration of the database, which by default is 4096 (a single memory page).
...
is policy for handling insertion ([`TS.ADD`](/content/commands/ts.add.md) and [`TS.MADD`](/content/commands/ts.madd.md)) of multiple samples with identical timestamps, with one of the following values:
  ...
  When not specified: set to the global [DUPLICATE_POLICY](/content/develop/data-types/timeseries/configuration.md#duplicate_policy) configuration of the database (which, by default, is `BLOCK`).
...
When not specified: set to the global [IGNORE_MAX_TIME_DIFF](/content/develop/data-types/timeseries/configuration.md#ignore_max_time_diff-and-ignore_max_val_diff) and [IGNORE_MAX_VAL_DIFF](/content/develop/data-types/timeseries/configuration.md#ignore_max_time_diff-and-ignore_max_val_diff), which are, by default, both set to 0.
</details>
...
* [Simple string reply](/content/develop/reference/protocol-spec.md#simple-strings): `OK` when the time series is created successfully.
* [Simple error reply](/content/develop/reference/protocol-spec.md#simple-errors) in these cases: invalid arguments, key already exists, etc.
...
[`TS.ADD`](/content/commands/ts.add.md) | [`TS.INCRBY`](/content/commands/ts.incrby.md) | [`TS.DECRBY`](/content/commands/ts.decrby.md) | [`TS.MGET`](/content/commands/ts.mget.md) | [`TS.MRANGE`](/content/commands/ts.mrange.md) | [`TS.MREVRANGE`](/content/commands/ts.mrevrange.md) | [`TS.QUERYINDEX`](/content/commands/ts.queryindex.md)
