...
the user.
The returned value can be synthesized back into a Redis key using the [`RESTORE`](/content/commands/restore.md)
command.
...
* It contains a 64-bit checksum that is used to make sure errors will be
  detected.
  The [`RESTORE`](/content/commands/restore.md) command makes sure to check the checksum before synthesizing a
...
In order to capture the time to live of the current value the [`PTTL`](/content/commands/pttl.md) command
should be used.
