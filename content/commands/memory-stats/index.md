...
title: MEMORY STATS
---
The `MEMORY STATS` command returns an [Array reply](/docs/reference/protocol-spec#arrays) about the memory usage of the
server.
...
*   `clients.normal`: The total size in bytes of all clients overheads (output
     and query buffers, connection contexts)
*   `cluster.links`: Memory usage by cluster links (Added in Redis 7.0, see [`INFO`](/commands/info)'s `mem_cluster_links`).
