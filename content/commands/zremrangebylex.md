...
The meaning of `min` and `max` are the same of the [`ZRANGEBYLEX`](/content/commands/zrangebylex.md) command. Similarly, this command actually removes the same elements that [`ZRANGEBYLEX`](/content/commands/zrangebylex.md) would return if called with the same `min` and `max` arguments.
