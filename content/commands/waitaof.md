...
2. If the command is sent as part of a [`MULTI`](/content/commands/multi.md) transaction (or any other context that does not allow blocking, such as inside scripts), the command does not block but instead returns immediately the number of masters and replicas that fsynced all previous write commands.
...
Note that, similarly to [`WAIT`](/content/commands/wait.md), `WAITAOF` does not make Redis a strongly-consistent store.
