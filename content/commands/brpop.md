...
It is the blocking version of [`RPOP`](/content/commands/rpop.md) because it blocks the connection when there
are no elements to pop from any of the given lists.
An element is popped from the tail of the first list that is non-empty, with the
given keys being checked in the order that they are given.

See the [BLPOP documentation](/content/commands/blpop.md) for the exact semantics, since `BRPOP` is
identical to [`BLPOP`](/content/commands/blpop.md) with the only difference being that it pops elements from
