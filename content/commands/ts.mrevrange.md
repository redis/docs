...
When combined with `AGGREGATION` the `GROUPBY`/`REDUCE` is applied post aggregation stage.
  ...
  - `reducer` is an aggregation type used to aggregate the results in each group.
    ...
    | `range`   | Difference between maximum non-NaN value and minimum non-NaN value (ignores NaN values). Added in RedisTimeSeries 1.8. |
    | `count`   | Number of non-NaN values (ignores NaN values). Added in RedisTimeSeries 1.8.                                  |
    | `countNaN`| Number of NaN values (since Redis 8.6)                                                         |
    | `countAll`| Number of all values, both NaN and non-NaN (since Redis 8.6)                                   |
    | `std.p`   | Population standard deviation of all non-NaN values (ignores NaN values). Added in RedisTimeSeries 1.8.       |
    | `std.s`   | Sample standard deviation of all non-NaN values (ignores NaN values). Added in RedisTimeSeries 1.8.           |
    | `var.p`   | Population variance of all non-NaN values (ignores NaN values). Added in RedisTimeSeries 1.8.                 |
    | `var.s`   | Sample variance of all non-NaN values (ignores NaN values). Added in RedisTimeSeries 1.8.                     |
