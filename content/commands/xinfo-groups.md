...
The stream's counter (the `entries_added` field of the [`XINFO STREAM`](/content/commands/xinfo-stream.md) command) is incremented by one with every [`XADD`](/content/commands/xadd.md) and counts all of the entries added to the stream during its lifetime.
...
1. A consumer group is created or set with an arbitrary last delivered ID (the [`XGROUP CREATE`](/content/commands/xgroup-create.md) and [`XGROUP SETID`](/content/commands/xgroup-setid.md) commands, respectively).
    An arbitrary ID is any ID that isn't the ID of the stream's first entry, its last entry or the zero ("0-0") ID.
2. One or more entries between the group's `last-delivered-id` and the stream's `last-generated-id` were deleted (with [`XDEL`](/content/commands/xdel.md) or a trimming operation).
