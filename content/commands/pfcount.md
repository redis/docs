...
For example, to take the count of all the unique search queries performed in a day, a program needs to call [`PFADD`](/content/commands/pfadd.md) every time a query is processed. The estimated number of unique queries can be retrieved with `PFCOUNT` at any time.
...
previously computed, that rarely changes because most [`PFADD`](/content/commands/pfadd.md) operations will
...
The HyperLogLog, being a Redis string, can be retrieved with [`GET`](/content/commands/get.md) and restored with [`SET`](/content/commands/set.md). Calling [`PFADD`](/content/commands/pfadd.md), `PFCOUNT` or [`PFMERGE`](/content/commands/pfmerge.md) commands with a corrupted HyperLogLog is never a problem, it may return random values but does not affect the stability of the server. Most of the times when corrupting a sparse representation, the server recognizes the corruption and returns an error.
