...
This command is analogous to the [`CLUSTER FAILOVER`](/content/commands/cluster-failover.md) command for non-clustered Redis and is similar to the failover support provided by sentinel.
...
If the previous master had additional replicas attached to it, they will continue replicating from it as chained replicas. You will need to manually execute a [`REPLICAOF`](/content/commands/replicaof.md) on these replicas to start replicating directly from the new master.
...
NOTE: [`REPLICAOF`](/content/commands/replicaof.md) is disabled while a failover is in progress, this is to prevent unintended interactions with the failover that might cause data loss.
