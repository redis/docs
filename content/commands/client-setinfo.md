...
---
The `CLIENT SETINFO` command assigns various info attributes to the current connection which are displayed in the output of [`CLIENT LIST`](/content/commands/client-list.md) and [`CLIENT INFO`](/content/commands/client-info.md).
...
There is no limit to the length of these attributes. However it is not possible to use spaces, newlines, or other non-printable characters that would violate the format of the [`CLIENT LIST`](/content/commands/client-list.md) reply.
...
Note that these attributes are not cleared by the [`RESET`](/content/commands/reset.md) command.
