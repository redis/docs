...
The above rule applies the new rule to the user `virginia`, so other than [`SET`](/content/commands/set.md), the user `virginia` can now also use the [`GET`](/content/commands/get.md) command.
...
The selector then grants a secondary set of permissions: access to the [`SET`](/content/commands/set.md) command to be executed on any key that starts with `app1`.
...
For information on persisting ACLs, see the [ACL tutorial](/content/operate/oss_and_stack/management/security/acl.md#use-an-external-acl-file).

The following documentation is a reference manual about the capabilities of this command, however our [ACL tutorial](/content/operate/oss_and_stack/management/security/acl.md) may be a more gentle introduction to how the ACL system works in general.
...
* `~<pattern>`: Adds the specified key pattern (glob style pattern, like in the [`KEYS`](/content/commands/keys.md) command), to the list of key patterns accessible by the user. This grants both read and write permissions to keys that match the pattern. You can add multiple key patterns to the same user. Example: `~objects:*`
* `%R~<pattern>`: (Available in Redis 7.0 and later) Adds the specified read key pattern. This behaves similar to the regular key pattern but only grants permission to read from keys that match the given pattern. See [key permissions](/content/operate/oss_and_stack/management/security/acl.md#key-permissions) for more information.
* `%W~<pattern>`: (Available in Redis 7.0 and later) Adds the specified write key pattern. This behaves similar to the regular key pattern but only grants permission to write to keys that match the given pattern. See [key permissions](/content/operate/oss_and_stack/management/security/acl.md#key-permissions) for more information.
...
* `+@<category>`: Adds all the commands in the specified category to the list of commands the user is able to execute. Example: `+@string` (adds all the string commands). For a list of categories, check the [`ACL CAT`](/content/commands/acl-cat.md) command.
...
* `off`: Set user as not active, it will be impossible to log as this user. Please note that if a user gets disabled (set to off) after there are connections already authenticated with such a user, the connections will continue to work as expected. To also kill the old connections you can use [`CLIENT KILL`](/content/commands/client-kill.md) with the user option. An alternative is to delete the user with [`ACL DELUSER`](/content/commands/acl-deluser.md), that will result in all the connections authenticated as the deleted user to be disconnected.
