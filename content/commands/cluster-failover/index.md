...
* A replica can only be promoted to a master if it is known as a replica by a majority of the masters in the cluster.
  ...
  To check that the masters are aware of a new replica, you can send [`CLUSTER NODES`](/commands/cluster-nodes) or [`CLUSTER REPLICAS`](/commands/cluster-replicas) to each of the master nodes and check that it appears as a replica, before sending `CLUSTER FAILOVER` to the replica.
