...
// REMOVE_END
{
    public function testQueryRange() {
        ...
        try { $r->ftdropindex('idx:bicycle'); } catch (Exception $e) {}
        $r->flushall();
        // REMOVE_END
        
        $schema = [
            new TextField('$.brand', 'brand'),
            new TextField('$.model', 'model'),
            new TextField('$.description', 'description'),
            new NumericField('$.price', 'price'),
            new TagField('$.condition', 'condition'),
        ];

        try {
            $r->ftCreate('idx:bicycle', $schema, (new CreateArguments())
                ->on('JSON')
                ->prefix(['bicycle:'])
            );
        } catch (Exception $e) {
        ...
        foreach ($bicycleDocs as $i => $doc) {
            $r->jsonset("bicycle:$i", '$', json_encode($doc, JSON_THROW_ON_ERROR));
        }

        // STEP_START range1
        $args1 = new SearchArguments();
        $args1->addReturn(1, 'price');
        $res1 = $r->ftSearch('idx:bicycle', '@price:[500 1000]', $args1);
        echo $res1[0] . PHP_EOL; // >>> 3

        $range1Map = [];
        for ($i = 1; $i < count($res1); $i += 2) {
            $id = $res1[$i];
            $fields = $res1[$i + 1];
            $priceIdx = array_search('price', $fields, true);
            $priceVal = $priceIdx !== false && isset($fields[$priceIdx + 1]) ? $fields[$priceIdx + 1] : null;
            $range1Map[$id] = $priceVal;
            echo $id . ' : price ' . $priceVal . PHP_EOL;
        }
        // >>> bicycle:2 : price 815
        // >>> bicycle:5 : price 810
        // >>> bicycle:9 : price 815
        // STEP_END
        // REMOVE_START
        $this->assertEquals(3, $res1[0]);
        $ids1 = array_keys($range1Map);
        sort($ids1);
        $this->assertEquals(['bicycle:2','bicycle:5','bicycle:9'], $ids1);
        $this->assertEquals('815', $range1Map['bicycle:2']);
        $this->assertEquals('810', $range1Map['bicycle:5']);
        $this->assertEquals('815', $range1Map['bicycle:9']);
        // REMOVE_END

        // STEP_START range2
        $args2 = new SearchArguments();
        $args2->addReturn(1, 'price');
        $args2->searchFilter(['price', 500, 1000]);
        $res2 = $r->ftSearch('idx:bicycle', '*', $args2);
        echo $res2[0] . PHP_EOL; // >>> 3

        $range2Map = [];
        for ($i = 1; $i < count($res2); $i += 2) {
            $id = $res2[$i];
            $fields = $res2[$i + 1];
            $priceIdx = array_search('price', $fields, true);
            $priceVal = $priceIdx !== false && isset($fields[$priceIdx + 1]) ? $fields[$priceIdx + 1] : null;
            $range2Map[$id] = $priceVal;
            echo $id . ' : price ' . $priceVal . PHP_EOL;
        }
        // >>> bicycle:2 : price 815
        // >>> bicycle:5 : price 810
        ...
        // STEP_END
        // REMOVE_START
        $this->assertEquals(3, $res2[0]);
        $ids2 = array_keys($range2Map);
        sort($ids2);
        $this->assertEquals(['bicycle:2','bicycle:5','bicycle:9'], $ids2);
        $this->assertEquals('815', $range2Map['bicycle:2']);
        $this->assertEquals('810', $range2Map['bicycle:5']);
        $this->assertEquals('815', $range2Map['bicycle:9']);
        // REMOVE_END

        // STEP_START range3
        $args3 = new SearchArguments();
        $args3->addReturn(1, 'price');
        // Use exclusive lower bound by prefixing with '(' per RediSearch range syntax
        $args3->searchFilter(['price', '(1000', '+inf']);
        $res3 = $r->ftSearch('idx:bicycle', '*', $args3);
        echo $res3[0] . PHP_EOL; // >>> 5

        $range3Map = [];
        for ($i = 1; $i < count($res3); $i += 2) {
            $id = $res3[$i];
            $fields = $res3[$i + 1];
            $priceIdx = array_search('price', $fields, true);
            $priceVal = $priceIdx !== false && isset($fields[$priceIdx + 1]) ? $fields[$priceIdx + 1] : null;
            $range3Map[$id] = $priceVal;
            echo $id . ' : price ' . $priceVal . PHP_EOL;
        }
        ...
        // STEP_END
        // REMOVE_START
        $this->assertEquals(5, $res3[0]);
        $ids3 = array_keys($range3Map);
        sort($ids3);
        $this->assertEquals(['bicycle:1','bicycle:3','bicycle:4','bicycle:6','bicycle:8'], $ids3);
        $this->assertEquals('1200', $range3Map['bicycle:1']);
        $this->assertEquals('2300', $range3Map['bicycle:6']);
        $this->assertEquals('3200', $range3Map['bicycle:4']);
        $this->assertEquals('3400', $range3Map['bicycle:3']);
        $this->assertEquals('1200', $range3Map['bicycle:8']);
        // REMOVE_END

        // STEP_START range4
        $args4 = new SearchArguments();
        $args4->addReturn(1, 'price');
        $args4->sortBy('price', 'asc');
        $args4->limit(0, 5);
        $res4 = $r->ftSearch('idx:bicycle', '@price:[-inf 2000]', $args4);
        echo $res4[0] . PHP_EOL; // >>> 7

        $range4Map = [];
        for ($i = 1; $i < count($res4); $i += 2) {
            $id = $res4[$i];
            $fields = $res4[$i + 1];
            $priceIdx = array_search('price', $fields, true);
            $priceVal = $priceIdx !== false && isset($fields[$priceIdx + 1]) ? $fields[$priceIdx + 1] : null;
            $range4Map[$id] = $priceVal;
            echo $id . ' : price ' . $priceVal . PHP_EOL;
        }
        // >>> bicycle:0 : price 270
        // >>> bicycle:7 : price 430
        ...
        // STEP_END
        // REMOVE_START
        $this->assertEquals(7, $res4[0]);
        $ids4 = array_keys($range4Map);
        sort($ids4);
        $this->assertEquals(['bicycle:0','bicycle:2','bicycle:5','bicycle:7','bicycle:9'], $ids4);
        $this->assertEquals('270', $range4Map['bicycle:0']);
        $this->assertEquals('430', $range4Map['bicycle:7']);
        $this->assertEquals('810', $range4Map['bicycle:5']);
        $this->assertEquals('815', $range4Map['bicycle:2']);
        $this->assertEquals('815', $range4Map['bicycle:9']);
        // REMOVE_END
    }
}
