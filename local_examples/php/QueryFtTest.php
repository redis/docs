...
// REMOVE_END
{
    public function testQueryFt() {
        ...
        // STEP_START ft1
        $res1 = $r->ftSearch('idx:bicycle', '@description: kids');
        echo $res1[0] . PHP_EOL; // >>> 2
        ...
        // STEP_END
        // REMOVE_START
        $this->assertEquals(2, $res1[0]);
        sort($docs1Ids);
        $this->assertEquals(['bicycle:1','bicycle:2'], $docs1Ids);
        // REMOVE_END

        // STEP_START ft2
        $res2 = $r->ftSearch('idx:bicycle', '@model: ka*');
        echo $res2[0] . PHP_EOL; // >>> 1
        ...
        // STEP_END
        // REMOVE_START
        $this->assertEquals(1, $res2[0]);
        $this->assertEquals('bicycle:4', $docs2Ids[0]);
        // REMOVE_END

        // STEP_START ft3
        $res3 = $r->ftSearch('idx:bicycle', '@brand: *bikes');
        echo $res3[0] . PHP_EOL; // >>> 2
        ...
        // REMOVE_START
        sort($docs3Ids);
        $this->assertEquals(2, $res3[0]);
        $this->assertEquals(['bicycle:4','bicycle:6'], $docs3Ids);
        // REMOVE_END

        // STEP_START ft4
        $res4 = $r->ftSearch('idx:bicycle', '%optamized%');
        echo $res4[0] . PHP_EOL; // >>> 1
        ...
        // STEP_END
        // REMOVE_START
        $this->assertEquals(1, $res4[0]);
        $this->assertEquals('bicycle:3', $docs4Ids[0]);
        // REMOVE_END

        // STEP_START ft5
        $res5 = $r->ftSearch('idx:bicycle', '%%optamised%%');
        echo $res5[0] . PHP_EOL; // >>> 1

        $docs5Ids = [];
        for ($i = 1; $i < count($res5); $i += 2) {
            $docs5Ids[] = $res5[$i];
            echo $res5[$i] . PHP_EOL;
        }
        // >>> bicycle:3
        // STEP_END
        // REMOVE_START
        $this->assertEquals(1, $res5[0]);
        $this->assertEquals('bicycle:3', $docs5Ids[0]);
        // REMOVE_END
    }
}
